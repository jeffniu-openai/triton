...
- Continue optimizing `python/examples/gluon/05-moe-bmm1-fused-gather.py` without voluntarily stopping while there are untested hypotheses, unmeasured regressions, or plausible 2CTA tuning paths. The target is to make the 2CTA kernel faster than 1CTA on every batch size, enable 2CTA for every batch size, then keep improving performance, especially for batch sizes `<= 2048`.
