// RUN: triton-opt %s -split-input-file -tritongpu-remove-layout-conversions | FileCheck %s --check-prefixes=NO-REORDER,TREE
// RUN: sed '/ALLOW-REORDER/s/ : tensor/ allow_reorder : tensor/' %s | triton-opt - -split-input-file -tritongpu-remove-layout-conversions | FileCheck %s --check-prefixes=ALLOW-REORDER,TREE

// This is reduced from a stochastic-rounding integer expansion built from
// reshape and join operations.  The pass reaches a fixed point with all eight
// conversions still present.  Marking the six indicated reshapes as
// allow_reorder removes all eight conversions.  Removing any one of the six
// annotations leaves one conversion.
//
// NO-REORDER-LABEL: @stochastic_rounding_join_chain
// NO-REORDER-COUNT-8: ttg.convert_layout
// NO-REORDER-NOT: ttg.convert_layout
// NO-REORDER: tt.return
//
// ALLOW-REORDER-LABEL: @stochastic_rounding_join_chain
// ALLOW-REORDER-NOT: ttg.convert_layout
// ALLOW-REORDER: tt.return
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func @stochastic_rounding_join_chain(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: i32) -> tensor<32x64xi32, #mma> {
    ...
    %4 = tt.reshape %3 : tensor<1x128x2xi32, #blocked4> -> tensor<256xi32, #blocked1> // ALLOW-REORDER
    ...
    %8 = tt.reshape %7 : tensor<1x128x2xi32, #blocked4> -> tensor<256xi32, #blocked1> // ALLOW-REORDER

    %9 = arith.xori %4, %8 : tensor<256xi32, #blocked1>
    %10 = tt.reshape %3 : tensor<1x128x2xi32, #blocked4> -> tensor<1x256xi32, #blocked2> // ALLOW-REORDER
    ...
    %18 = tt.reshape %17 : tensor<1x128x2xi32, #blocked4> -> tensor<1x256xi32, #blocked2> // ALLOW-REORDER
    ...
    %30 = tt.reshape %29 : tensor<1x128x2xi32, #blocked4> -> tensor<1x256xi32, #blocked2> // ALLOW-REORDER
    ...
    %40 = tt.reshape %39 : tensor<1x1024x2xi32, #linear7> -> tensor<32x64xi32, #blocked> // ALLOW-REORDER
...
// This is reduced from https://github.com/triton-lang/triton/issues/4030.
// Current main moves fourteen conversions through the join tree and leaves one
// conversion after the final reshape, where the fixed result layout requires
// a different blocked encoding.
//
