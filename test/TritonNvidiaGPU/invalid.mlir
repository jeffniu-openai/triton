...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.shared = 65536 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func @tmem_copy_scales_descriptor_family_clean_unsupported(
    ...
    // expected-note @+3 {{tcgen05.copy.warpx4.32x128b descriptor message 0 has an unsupported instruction-column projection. Within one 16-column tcgen05.copy instruction, source column bit 1 maps to shared offset 8 (2 descriptor-row strides), which would require this column bit to select a different descriptor row within the same instruction footprint instead of contiguous shared offset 4. Public tcgen05.copy takes one tensor-memory address and one shared descriptor per instruction and has no per-column destination mask, so this projection needs a different copy atom, source format, or a proven multi-message schedule that avoids overwriting unrelated destination columns before it can be supported.}}
