...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.shared = 65536 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func @tmem_copy_scales_descriptor_family_clean_unsupported(
    ...
    // expected-note @+3 {{tcgen05.copy.warpx4.32x128b descriptor message 0 has an unsupported instruction-column projection. Within one 16-column tcgen05.copy instruction, source column bit 1 maps to shared offset 8 instead of contiguous shared offset 4. Current copy scheduling cannot split sub-instruction source columns, so this projection needs a different copy atom or a multi-message schedule before it can be supported.}}
...
module attributes {"ttg.num-ctas" = 2 : i32, "ttng.two-ctas" = true, "ttg.num-warps" = 4 : i32, ttg.shared = 65536 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @tmem_copy_no_scales_warpx2_02_13_twocta_clean_unsupported(
    ...
    // expected-note @+3 {{The two-CTA warpx2::02_13 path remains unsupported until Triton can synthesize a cta_group::2 descriptor/address schedule that preserves the high source-column bit; direct-seed cta_group::2 probes emit the opcode but duplicate the low source-column pair, while the aligned dword deltas that complete the single-CTA schedule read zeros under cta_group::2. Decomposing this tensor-memory view into cta_group::1 copies is not valid because two-CTA TMEM allocation uses cta_group::2 granularity.}}
