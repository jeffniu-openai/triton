...
// -----
#tmem64 = #ttng.tensor_memory_encoding<blockM = 64, blockN = 64, colStride = 1>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.shared = 65536 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @tmem_alloc_result_alloc_shape_mismatch() {
    // expected-error @+1 {{result shape and its alloc shape must match}}
    %0 = ttng.tmem_alloc : () -> !ttg.memdesc<64x64xf32, #tmem64, #ttng.tensor_memory, mutable, 128x128>
    tt.return
  }
}
...
#tmem_barrier = #ttng.tensor_memory_encoding<blockM = 128, blockN = 2, colStride = 1>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32} {
  tt.func @tcgen5_commit_barrier_memory_space(
      %bar: !ttg.memdesc<128x2xf32, #tmem_barrier, #ttng.tensor_memory, mutable>,
      %pred: i1) {
    // expected-error @below {{barrier allocation must be a shared memory descriptor}}
    ttng.tc_gen5_commit %bar, %pred : !ttg.memdesc<128x2xf32, #tmem_barrier, #ttng.tensor_memory, mutable>
    tt.return
  }
}
