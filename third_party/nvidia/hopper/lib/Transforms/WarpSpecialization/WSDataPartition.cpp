  ...
  for (auto opDim : partitionScheme.rematerializedOps) {
    ...
    for (unsigned i = 1; i < opDim.second.size(); i++) {
      ...
      if (auto allocOp = dyn_cast<LocalAllocOp>(oldOp)) {
        ...
        auto viewOp = MemDescSubsliceOp::createChecked(
            builder, allocOp.getLoc(), allocOp.getResult(), shape, offsets);
        assert(succeeded(viewOp) && "expected valid memdesc_subslice");
        newOp = *viewOp;
