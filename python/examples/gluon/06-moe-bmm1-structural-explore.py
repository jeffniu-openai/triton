...
epilogue_wide_store_handoff = ex.epilogue_wide_store_handoff
_store_out_subtile = ex._store_out_subtile
...
@gluon.jit
def load_inputs_x2n4_partition(p: PartitionArgs):
    local_cga_layout: gl.constexpr = ((0, 0), )
    offs_layout: gl.constexpr = gl.SliceLayout(
        dim=0,
        parent=gl.BlockedLayout([1, 4], [32, 1], [1, gl.num_warps()], [1, 0], cga_layout=local_cga_layout),
    ...
    w_phase = 1
    w_issued = 0
    quad_grid_n: gl.constexpr = triton.cdiv(p.GRID_N, 4)

    for block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        schedule_pid_m, quad_pid_n = banded_row_major(block_id, p.grid_m, quad_grid_n, BAND_N=p.BAND_N)
        slice_idx, pid_m = unpack_block_schedule(gl.load(p.x_block_schedule + schedule_pid_m))
        rank = gl.cluster_cta_rank()
        pid_n0 = (quad_pid_n.to(gl.int32) * 4 + rank.to(gl.int32) * 2).to(gl.int32)
        pid_n1 = pid_n0 + 1
        active0 = pid_n0 < p.GRID_N
        active1 = pid_n1 < p.GRID_N
        slice_offset = gl.load(p.x_slice_offs + slice_idx)
        off_m = pid_m * p.BLOCK_M
        shape_m = gl.load(p.x_slice_sizes + slice_idx)
        offs_m = off_m + gl.arange(0, p.BLOCK_M, layout=offs_layout)
        ...
        scale_idx0 = slice_idx * p.SCALE_FLAT_N + pid_n0 * p.SCALE_BLOCK_N_DIV
        scale_idx1 = scale_idx0 + p.SCALE_BLOCK_N_DIV
        ...
        for ki in range(p.K_TILES):
            ...
            if active0:
                ...
                mbarrier.wait(w_empty_bar, w_phase, pred=w_issued >= p.w_num_bufs)
                mbarrier.expect(w_ready_bar, bytes_per_w_stage)
                tma.async_copy_global_to_shared(p.w_desc, [slice_idx, ki, pid_n0, 0, 0], w_ready_bar, w_buf)
                tma.async_copy_global_to_shared(
                    p.scale_desc,
                    [0, scale_idx0, off_k_scale, 0, 0],
                    w_ready_bar,
                    scale_buf,
                    multicast=False,
                )
                ...
                if active1:
                    ...
                    mbarrier.wait(next_w_empty_bar, next_w_phase, pred=(w_issued + 1) >= p.w_num_bufs)
                    mbarrier.expect(next_w_ready_bar, bytes_per_w_stage)
                    tma.async_copy_global_to_shared(
                        p.w_desc,
                        [slice_idx, ki, pid_n1, 0, 0],
                        next_w_ready_bar,
                        next_w_buf,
                    )
                    tma.async_copy_global_to_shared(
                        p.scale_desc,
                        [0, scale_idx1, off_k_scale, 0, 0],
                        next_w_ready_bar,
                        next_scale_buf,
                        multicast=False,
                    )
...
@gluon.jit
def mma_x2n4_compute_partition(p: PartitionArgs):
    x_idx = 0
    x_phase = 0
    w_idx = 0
    w_phase = 0
    ...
    for block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        ...
        if active0:
            ...
            mbarrier.wait(next_acc_empty_bar, next_mma_phase, pred=active1)
            ...
            for _ in range(p.K_TILES):
                ...
                if active1:
                    ...
                    blackwell.tcgen05_copy(
                        unswizzle_mx_scale(next_scale_buf, p.SCALE_SIZE_OUTER, p.SCALE_SIZE_INNER, p.MXFP_BLOCK_SIZE),
                        p.w_scale_tmem_alt,
                    )
                    blackwell.tcgen05_mma_scaled(
                ...
                blackwell.tcgen05_commit(x_empty_bar, descs=[x_buf])
                blackwell.tcgen05_commit(x_empty_bar, descs=[x_buf], pred=active1)
                x_idx, x_phase = advance(x_idx, x_phase, p.x_num_bufs)
                use_acc = True

            blackwell.tcgen05_commit(acc_ready_bar)
            blackwell.tcgen05_commit(next_acc_ready_bar, pred=active1)
            if active1:
...
@gluon.jit
def mma_x2n_natural_compute_partition(p: PartitionArgs):
    x_idx = 0
    x_phase = 0
    w_idx = 0
    w_phase = 0
    ...
    for block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        _, pair_pid_n = banded_row_major(block_id, p.grid_m, pair_grid_n, BAND_N=p.BAND_N)
        rank = gl.cluster_cta_rank()
        pid_n = (pair_pid_n.to(gl.int32) * 2 + rank).to(gl.int32)
        active = pid_n < p.GRID_N
        if active:
...
@gluon.jit
def epilogue_x2n4_partition(p: PartitionArgs):
    idx = 0
    phase = 0
    quad_grid_n: gl.constexpr = triton.cdiv(p.GRID_N, 4)
    ...
    for block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        ...
        off_m = pid_m * p.BLOCK_M
        shape_m = gl.load(p.x_slice_sizes + slice_idx)
        for local_n in gl.static_range(2):
            pid_n = pid_n0 + local_n
            active = pid_n < p.GRID_N
            if active:
                out_off_n_packed = pid_n * (p.BLOCK_N // p.REDUCTION_N // 4)
                idx, phase, acc_packed = apply_bias_and_scale(
...
@gluon.jit
def epilogue_x2n4_handoff_partition(p: PartitionArgs):
    idx = 0
    phase = 0
    store_idx = 0
    store_phase = 1
        ...
        for local_n in gl.static_range(2):
            ...
            if active:
                if p.USE_WIDE_STORE_HANDOFF:
                    ...
                    gl.static_assert(p.SWIGLU_SUBTILE_FACTOR > 1, "x2n4 helper handoff requires M fragments")
                    ...
                    for frag_idx in gl.static_range(p.SWIGLU_SUBTILE_FACTOR):
                        ...
                        acc_regs = acc_sub.load().permute((1, 0))
...
@gluon.jit
def epilogue_x2n_natfrag_partition(p: PartitionArgs):
    idx = 0
    phase = 0
    pair_grid_n: gl.constexpr = triton.cdiv(p.GRID_N, 2)

    gl.static_assert(p.USE_DIRECT_EPILOGUE_STORE, "natural fragment epilogue writes directly")
    gl.static_assert(p.BLOCK_N % p.SWIGLU_SUBTILE_FACTOR == 0, "natural fragment epilogue requires even N split")
    frag_cols: gl.constexpr = p.BLOCK_N // p.SWIGLU_SUBTILE_FACTOR
    gl.static_assert(frag_cols % (p.REDUCTION_N * 4) == 0, "N fragments must align with packed output columns")
    ...
    num_warps: gl.constexpr = gl.num_warps()
    warps_n: gl.constexpr = 1 if p.FORCE_EPILOGUE_WARPS_N1 else (2 if num_warps >= 8 and frag_cols >= 128 else 1)
    local_cga_layout: gl.constexpr = ((0, 0), )
    frag_split_layout: gl.constexpr = gl.BlockedLayout(
        [1, 4],
        [1, 32],
    ...
    for block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        ...
        if active:
            ...
            for frag_idx in gl.static_range(p.SWIGLU_SUBTILE_FACTOR):
                acc_sub = acc_buf.slice(frag_idx * frag_cols, frag_cols)
                acc_regs = acc_sub.load()
                if frag_idx == p.SWIGLU_SUBTILE_FACTOR - 1:
                    mbarrier.arrive(acc_empty_bar)

                acc = gl.convert_layout(acc_regs, frag_split_layout)
                offs_bias_n = off_n + frag_idx * frag_cols + gl.arange(0, frag_cols, layout=frag_bias_layout)
                bias = gl.convert_layout(
                    gl.expand_dims(gl.load(p.bias_ptr + slice_idx * p.bias_stride + offs_bias_n), axis=0),
                    frag_split_layout,
                )
                ...
                store_packed_out(
                    ...
                    packed_fp8,
                    off_m,
                    out_off_n_packed + frag_idx * (frag_cols // p.REDUCTION_N // 4),
                    shape_m,
                    slice_offset,
                )
            idx = next_idx
...
@gluon.jit
def epilogue_x2n_permfrag_partition(p: PartitionArgs):
    idx = 0
    phase = 0
    pair_grid_n: gl.constexpr = triton.cdiv(p.GRID_N, 2)

    gl.static_assert(p.USE_DIRECT_EPILOGUE_STORE, "permuted fragment epilogue writes directly")
    gl.static_assert(p.BLOCK_N % p.SWIGLU_SUBTILE_FACTOR == 0, "permuted fragment epilogue requires even N split")
    frag_cols: gl.constexpr = p.BLOCK_N // p.SWIGLU_SUBTILE_FACTOR
    gl.static_assert(frag_cols % (p.REDUCTION_N * 4) == 0, "N fragments must align with packed output columns")
    ...
    gl.static_assert(frag_cols == 64 or frag_cols == 128, "permfrag TMEM fragment blockM must be 64 or 128")
    frag_tmem_layout: gl.constexpr = blackwell.TensorMemoryLayout(
        [frag_cols, p.BLOCK_M],
        col_stride=1,
        cga_layout=local_cga_layout,
        two_ctas=False,
    ...
    for block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        ...
        if active:
            ...
            acc_ready_bar = p.acc_ready_bars.index(idx)
            acc_buf = p.acc_bufs.index(idx)
            acc_view = acc_buf._reinterpret(
                gl.float32,
                [p.SWIGLU_SUBTILE_FACTOR, frag_cols, p.BLOCK_M],
                frag_tmem_layout,
            )
            ...
            for frag_idx in gl.static_range(p.SWIGLU_SUBTILE_FACTOR):
                acc_sub = acc_view.index(gl.full((), frag_idx, gl.int32))
                acc_regs = acc_sub.load().permute((1, 0))
                acc = gl.convert_layout(acc_regs, frag_split_layout)
                if frag_idx == p.SWIGLU_SUBTILE_FACTOR - 1:
                    mbarrier.arrive(acc_empty_bar)
...
@gluon.jit
def get_x2n_store_layout(p: PartitionArgs):
    frag_rows: gl.constexpr = p.BLOCK_M // p.SWIGLU_SUBTILE_FACTOR
...
@gluon.jit
def epilogue_x2n4_store_partition(p: PartitionArgs):
    gl.static_assert(p.SWIGLU_SUBTILE_FACTOR > 1, "x2n4 store helper requires row fragments")
    store_layout: gl.constexpr = get_x2n_store_layout(p)
    frag_rows: gl.constexpr = p.BLOCK_M // p.SWIGLU_SUBTILE_FACTOR
    quad_grid_n: gl.constexpr = triton.cdiv(p.GRID_N, 4)
...
@gluon.jit
def epilogue_x2n_store_partition(p: PartitionArgs):
    gl.static_assert(p.SWIGLU_SUBTILE_FACTOR > 1, "x2n helper store requires row fragments")
    store_layout: gl.constexpr = get_x2n_store_layout(p)
    frag_rows: gl.constexpr = p.BLOCK_M // p.SWIGLU_SUBTILE_FACTOR
    pair_grid_n: gl.constexpr = triton.cdiv(p.GRID_N, 2)
...
@gluon.jit
def epilogue_x2n_handoff_partition(p: PartitionArgs):
    idx = 0
    phase = 0
    store_idx = 0
    store_phase = 1
    ...
    for block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        ...
        if active:
            if p.USE_WIDE_STORE_HANDOFF:
                ...
                gl.static_assert(p.SWIGLU_SUBTILE_FACTOR > 1, "fragment helper handoff requires M fragments")
                frag_rows: gl.constexpr = p.BLOCK_M // p.SWIGLU_SUBTILE_FACTOR
                acc_empty_bar = p.acc_empty_bars.index(idx)
                acc_ready_bar = p.acc_ready_bars.index(idx)
                acc_buf = p.acc_bufs.index(idx)
...
@gluon.jit
def load_inputs_cta_linear_partition(p: PartitionArgs):
    local_cga_layout: gl.constexpr = ((0, 0), )
    offs_layout: gl.constexpr = gl.SliceLayout(
        dim=0,
        parent=gl.BlockedLayout([1, 4], [32, 1], [1, gl.num_warps()], [1, 0], cga_layout=local_cga_layout),
    ...
    for cluster_block_id in range(gl.program_id(0), p.num_blocks, p.NUM_SMS):
        rank = gl.cluster_cta_rank()
        block_id = cluster_block_id * 2 + rank
        total_blocks = p.grid_m * p.GRID_N
        active = block_id < total_blocks
        if active:
            pid_m, pid_n, slice_idx, slice_offset = p.apply_block_schedule(block_id)
            off_m = pid_m * p.BLOCK_M
            shape_m = gl.load(p.x_slice_sizes + slice_idx)
...
@gluon.jit
def mma_cta_linear_partition(p: PartitionArgs):
    x_idx = 0
    x_phase = 0
    w_idx = 0
    w_phase = 0
...
@gluon.jit
def epilogue_cta_linear_partition(p: PartitionArgs):
    idx = 0
    phase = 0
    store_idx = 0
    store_phase = 1
...
@gluon.jit
def epilogue_cta_linear_store_partition(p: PartitionArgs):
    gl.static_assert(p.SWIGLU_SUBTILE_FACTOR > 1, "ctalinear store helper requires row fragments")
    store_layout: gl.constexpr = get_x2n_store_layout(p)
    frag_rows: gl.constexpr = p.BLOCK_M // p.SWIGLU_SUBTILE_FACTOR
...
@gluon.jit
def epilogue_cta_mpair_permfrag_partition(p: PartitionArgs):
    idx = 0
    phase = 0

    gl.static_assert(p.USE_DIRECT_EPILOGUE_STORE, "M-pair permfrag epilogue writes directly")
    gl.static_assert(p.BLOCK_N % p.SWIGLU_SUBTILE_FACTOR == 0, "M-pair permfrag epilogue requires even N split")
    frag_cols: gl.constexpr = p.BLOCK_N // p.SWIGLU_SUBTILE_FACTOR
    gl.static_assert(frag_cols % (p.REDUCTION_N * 4) == 0, "N fragments must align with packed output columns")
    ...
    gl.static_assert(frag_cols == 64 or frag_cols == 128, "M-pair permfrag TMEM fragment blockM must be 64 or 128")
    frag_tmem_layout: gl.constexpr = blackwell.TensorMemoryLayout(
        [frag_cols, p.BLOCK_M],
        col_stride=1,
        cga_layout=local_cga_layout,
...
def ws_matmul_combined_load_kernel(
    ...
    x2n_helper: gl.constexpr = STRUCTURAL_MODE == 53
    cta_linear: gl.constexpr = STRUCTURAL_MODE == 55
    x2n4: gl.constexpr = STRUCTURAL_MODE == 57
    x2n4_helper: gl.constexpr = x2n4 and not USE_DIRECT_EPILOGUE_STORE
    mma_store_handoff: gl.constexpr = STRUCTURAL_MODE == 35 or x2n_helper or cta_linear or x2n4_helper
    ...
    x2n_natfrag: gl.constexpr = STRUCTURAL_MODE == 52
    x2n_mmaepi: gl.constexpr = STRUCTURAL_MODE == 54
    x2n_permfrag: gl.constexpr = STRUCTURAL_MODE == 56
    ...
    scheduled_grid_n: gl.constexpr = triton.cdiv(grid_n, 4) if x2n4 else (triton.cdiv(grid_n, 2) if cta_npair_like else grid_n)
    scheduled_grid_m = (grid_m + 1) // 2 if cta_mpair else grid_m
    num_blocks = (scheduled_grid_m * scheduled_grid_n + 1) // 2 if cta_linear else scheduled_grid_m * scheduled_grid_n

    scale_k: gl.constexpr = BLOCK_K // MXFP_BLOCK_SIZE
    natural_acc: gl.constexpr = STRUCTURAL_MODE == 14
    natural_layout_acc: gl.constexpr = natural_acc or x2n_natfrag
    fake_nsplit: gl.constexpr = STRUCTURAL_MODE == 15
    needs_alt_w_scale: gl.constexpr = STRUCTURAL_MODE == 18 or STRUCTURAL_MODE == 20 or x2n4
    uses_2cta_layout: gl.constexpr = use_2cta
    local_2cta_layout: gl.constexpr = (
        cta_npair_like or cta_mpair or cta_linear or fake_nsplit or fakens_pairw or fakens_mmaepi
    )
    local_w_scale_layout: gl.constexpr = (
        cta_npair_like or cta_mpair or cta_linear or fake_nsplit or fakens_pairw or fakens_mmaepi
    )
    mma_two_ctas: gl.constexpr = uses_2cta_layout and not local_2cta_layout
    block_m_per_cta: gl.constexpr = BLOCK_M if cta_npair_like or cta_mpair or cta_linear else BLOCK_M // gl.num_ctas()
    gl.static_assert(
    ...
    shared_x_multicast: gl.constexpr = (
        x2n_fused or x2n4 or x2n_mfrag or x2n_natfrag or x2n_helper or x2n_mmaepi or x2n_permfrag
    )
    ...
    if natural_layout_acc:
        natural_cga_layout: gl.constexpr = ((0, 0), ) if x2n_natfrag else (((1, 0), ) if uses_2cta_layout else ())
        natural_two_ctas: gl.constexpr = False if x2n_natfrag else mma_two_ctas
        natural_block_m: gl.constexpr = BLOCK_M if x2n_natfrag else 128
        gl.static_assert(not (x2n_natfrag and BLOCK_M == 64), "tcgen05_mma_scaled rejects natural blockM=64")
        acc_layout: gl.constexpr = blackwell.TensorMemoryLayout(
            [natural_block_m, BLOCK_N],
            col_stride=1,
            cga_layout=natural_cga_layout,
            two_ctas=natural_two_ctas,
        )
        ...
        mma_block_col: gl.constexpr = min(
            128, BLOCK_N if cta_npair_like or cta_mpair or cta_linear else BLOCK_N // gl.num_ctas()
        )
        acc_layout: gl.constexpr = blackwell.TensorMemoryLayout(
    ...
    if USE_DIRECT_EPILOGUE_STORE:
        ...
        store_cga_layout: gl.constexpr = ((0, 0), ) if local_2cta_layout else (((0, 1), ) if use_2cta else ())
        store_bufs = gl.allocate_shared_memory(
            gl.int16,
            [EPILOGUE_BUFFER_DEPTH, store_rows, out_packed_n],
            gl.SwizzledSharedLayout(1, 1, 1, [1, 0], cga_layout=store_cga_layout),
        )
    ...
    elif STRUCTURAL_MODE == 57:
        gl.static_assert(gl.num_ctas() == 2, "x2n4 requires a 2CTA launch")
        gl.static_assert(X_GATHER_MULTICAST, "x2n4 depends on multicast activation gather")
        gl.static_assert(ACC_NUM_BUFS >= 2, "x2n4 needs two accumulator buffers")
        gl.static_assert(W_NUM_BUFS >= 2, "x2n4 needs two weight buffers")
        if USE_DIRECT_EPILOGUE_STORE:
            gl.warp_specialize(
                [
                    (noop_partition, (p, )),
                    (epilogue_x2n4_partition, (p, )),
                    (load_inputs_x2n4_partition, (p, )),
                    (mma_x2n4_compute_partition, (p, )),
                ],
                [STORE_HELPER_WARPS, LOAD_ACTIVATION_WARPS, MMA_WARPS],
                [STORE_HELPER_REGS, LOAD_ACTIVATION_REGS, MMA_REGS],
            )
        else:
            gl.static_assert(SWIGLU_SUBTILE_FACTOR > 1, "x2n4 helper needs fragmented M handoff")
            gl.warp_specialize(
                [
                    (noop_partition, (p, )),
                    (epilogue_x2n4_store_partition, (p, )),
                    (epilogue_x2n4_handoff_partition, (p, )),
                    (load_inputs_x2n4_partition, (p, )),
                    (mma_x2n4_compute_partition, (p, )),
                ],
                [STORE_HELPER_WARPS, LOAD_WEIGHT_WARPS, LOAD_ACTIVATION_WARPS, MMA_WARPS],
                [STORE_HELPER_REGS, LOAD_WEIGHT_REGS, LOAD_ACTIVATION_REGS, MMA_REGS],
            )
    ...
    elif STRUCTURAL_MODE == 52:
        gl.static_assert(USE_DIRECT_EPILOGUE_STORE, "x2n_natfrag uses local direct epilogue stores")
        gl.static_assert(gl.num_ctas() == 2, "x2n_natfrag requires a 2CTA launch")
        gl.static_assert(X_GATHER_MULTICAST, "x2n_natfrag depends on multicast activation gather")
        gl.warp_specialize(
            [
                (noop_partition, (p, )),
                (epilogue_x2n_natfrag_partition, (p, )),
                (load_inputs_x2n_fused_partition, (p, )),
                (mma_x2n_natural_compute_partition, (p, )),
            ],
            [STORE_HELPER_WARPS, LOAD_ACTIVATION_WARPS, MMA_WARPS],
            [STORE_HELPER_REGS, LOAD_ACTIVATION_REGS, MMA_REGS],
        )
    elif STRUCTURAL_MODE == 53:
        gl.static_assert(not USE_DIRECT_EPILOGUE_STORE, "x2n_helper uses the store helper handoff path")
        gl.static_assert(gl.num_ctas() == 2, "x2n_helper requires a 2CTA launch")
        gl.static_assert(X_GATHER_MULTICAST, "x2n_helper depends on multicast activation gather")
        gl.warp_specialize(
            [
                (noop_partition, (p, )),
                (epilogue_x2n_store_partition, (p, )),
                (epilogue_x2n_handoff_partition, (p, )),
                (load_inputs_x2n_fused_partition, (p, )),
                (mma_x2n_compute_partition, (p, )),
            ],
            [STORE_HELPER_WARPS, LOAD_WEIGHT_WARPS, LOAD_ACTIVATION_WARPS, MMA_WARPS],
    ...
    elif STRUCTURAL_MODE == 54:
        gl.static_assert(USE_DIRECT_EPILOGUE_STORE, "x2n_mmaepi uses local direct epilogue stores")
        gl.static_assert(gl.num_ctas() == 2, "x2n_mmaepi requires a 2CTA launch")
        gl.static_assert(X_GATHER_MULTICAST, "x2n_mmaepi depends on multicast activation gather")
        gl.warp_specialize(
            [
                (noop_partition, (p, )),
                (load_inputs_x2n_fused_partition, (p, )),
                (mma_x2n_fused_partition, (p, )),
            ],
            [LOAD_ACTIVATION_WARPS, MMA_WARPS],
            [LOAD_ACTIVATION_REGS, MMA_REGS],
        )
    elif STRUCTURAL_MODE == 56:
        gl.static_assert(USE_DIRECT_EPILOGUE_STORE, "x2n_permfrag uses local direct epilogue stores")
        gl.static_assert(gl.num_ctas() == 2, "x2n_permfrag requires a 2CTA launch")
        gl.static_assert(X_GATHER_MULTICAST, "x2n_permfrag depends on multicast activation gather")
        gl.warp_specialize(
            [
                (noop_partition, (p, )),
                (epilogue_x2n_permfrag_partition, (p, )),
                (load_inputs_x2n_fused_partition, (p, )),
                (mma_x2n_compute_partition, (p, )),
            ],
            [STORE_HELPER_WARPS, LOAD_ACTIVATION_WARPS, MMA_WARPS],
    ...
    elif STRUCTURAL_MODE == 55:
        gl.static_assert(gl.num_ctas() == 2, "ctalinear requires a 2CTA launch")
        if USE_DIRECT_EPILOGUE_STORE:
            gl.warp_specialize(
                [
                    (noop_partition, (p, )),
                    (epilogue_cta_linear_partition, (p, )),
                    (load_inputs_cta_linear_partition, (p, )),
                    (mma_cta_linear_partition, (p, )),
                ],
                [STORE_HELPER_WARPS, LOAD_ACTIVATION_WARPS, MMA_WARPS],
                [STORE_HELPER_REGS, LOAD_ACTIVATION_REGS, MMA_REGS],
            )
            ...
            gl.warp_specialize(
                [
                    (noop_partition, (p, )),
                    (epilogue_cta_linear_store_partition, (p, )),
                    (epilogue_cta_linear_partition, (p, )),
                    (load_inputs_cta_linear_partition, (p, )),
                    (mma_cta_linear_partition, (p, )),
                ],
    ...
    elif STRUCTURAL_MODE == 58:
        gl.static_assert(USE_DIRECT_EPILOGUE_STORE, "ctampair_permfrag uses local direct epilogue stores")
        gl.static_assert(gl.num_ctas() == 2, "ctampair_permfrag requires a 2CTA launch")
        gl.static_assert(STORE_HELPER_WARPS >= 4, "ctampair_permfrag epilogue TMEM loads require at least four store-helper warps")
        gl.warp_specialize(
            [
                (noop_partition, (p, )),
                (epilogue_cta_mpair_permfrag_partition, (p, )),
                (load_inputs_cta_mpair_partition, (p, )),
                (mma_cta_mpair_compute_partition, (p, )),
            ],
            [STORE_HELPER_WARPS, LOAD_ACTIVATION_WARPS, MMA_WARPS],
...
def combined_load_matmul(
    ...
    schedule_blocks = schedule_grid_m * schedule_grid_n
    if structural_mode == 55:
        schedule_blocks = triton.cdiv(schedule_blocks, 2)
    sms = torch.cuda.get_device_properties(bias.device).multi_processor_count
    sms *= p.OCCUPANCY
    launch_grid = max(1, min(max(1, sms // p.NUM_CTAS), schedule_blocks))
    if route_class_split:
...
def parse_candidate(name: str, slice_size: int):
    if name.startswith("combined:"):
        ...
        mode = "x2n_fused"
    elif name.startswith("x2n4:"):
        spec = name[len("x2n4:"):]
        mode = "x2n4"
    elif name.startswith("x2n_mfrag:"):
        spec = name[len("x2n_mfrag:"):]
        mode = "x2n_mfrag"
    elif name.startswith("x2n_natfrag:"):
        spec = name[len("x2n_natfrag:"):]
        mode = "x2n_natfrag"
    elif name.startswith("x2n_helper:"):
        spec = name[len("x2n_helper:"):]
        mode = "x2n_helper"
    elif name.startswith("x2n_mmaepi:"):
        spec = name[len("x2n_mmaepi:"):]
        mode = "x2n_mmaepi"
    elif name.startswith("x2n_permfrag:"):
        spec = name[len("x2n_permfrag:"):]
        mode = "x2n_permfrag"
    elif name.startswith("ctalinear:"):
        spec = name[len("ctalinear:"):]
        mode = "ctalinear"
    ...
    if spec is not None:
        ...
        if mode == "mmastore":
            ...
            updates.setdefault(
                "SWIGLU_SUBTILE_FACTOR",
                4 if mode == "x2n_natfrag" or mode == "x2n_mfrag" or mode == "x2n_permfrag" or mode == "ctampair_permfrag" else (
                    2 if mode == "x2n_helper" or x2n4_helper_config else 1
                ),
            )
            if mode == "x2n_helper" or x2n4_helper_config:
                updates.setdefault("EPILOGUE_BUFFER_DEPTH", 2)
            if x2n4_helper_config:
                updates.setdefault("LOAD_WEIGHT_WARPS", 4)
                updates.setdefault("LOAD_WEIGHT_REGS", 32)
            ...
            if mode in ("ctapair2", "ctampair2", "ctampair_permfrag"):
                ...
                or mode == "x2n_helper"
                or mode == "x2n_mmaepi"
                or mode == "x2n_permfrag"
                or mode == "ctalinear"
            ):
                updates.setdefault("NUM_WARPS", 8)
                updates.setdefault("STORE_HELPER_WARPS", 1 if mode == "x2n_helper" or x2n4_helper_config else 4)
                updates.setdefault("STORE_HELPER_REGS", 32)
        else:
