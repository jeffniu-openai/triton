...
def _borrow_s_as_alpha(config, s_tmem):
    ...
    return alpha_tmem.bitcast(gl.float32, [config.SPLIT_M, 1], alpha_layout)
...
def _borrow_s_for_epilogue(config, s_tmem):
    ...
    m_i_tmem = m_i_tmem.bitcast(gl.float32, [config.SPLIT_M, 1], layout)
    l_i_tmem = l_i_tmem.bitcast(gl.float32, [config.SPLIT_M, 1], layout)
...
test_providers = ["triton-fp16", "triton-bf16", "triton-fp8"]
...
@pytest.mark.parametrize("Z", TEST_BATCH)
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, provider, use_tmem_red, cta_layout, profile=False):
    device = "cuda"
