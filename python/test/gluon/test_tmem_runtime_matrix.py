...
SCALED_MMA_ACC_SUBSLICE_USE_ACC_CASES = [
    (a_format, b_format, n, slice_start)
...
def test_tmem_runtime_matrix_mma_lhs_tile_permuted(kind, n, k, tile_n):
    ...
    compiled = tmem_mma_lhs_kernel[(1, )](
        a, b, out, lhs_layout, acc_layout, shared_layout_b, n, k, num_warps=4
    )
...
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell")
@pytest.mark.parametrize("a_format,b_format,n,slice_start", SCALED_MMA_ACC_SUBSLICE_USE_ACC_CASES)
def test_tmem_runtime_matrix_mma_scaled_acc_subslice_view_format_use_acc(a_format, b_format, n, slice_start):
    m = 128
    k = 128
    acc_init = 1.0
    vec_size = 16 if a_format == "nvfp4" else 32
