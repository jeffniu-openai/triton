...
def tmem_ldst_dynamic_linear_subslice_view_kernel(in_ptr, out_ptr, selector_ptr,
    ...
    view0.store(value + ttgl.full([M, N], 3, element_ty, layout=reg_layout))
    view1.store(value + ttgl.full([M, N], 7, element_ty, layout=reg_layout))
...
def tmem_ldst_unaligned_subword_linear_subslice_view_kernel(in_ptr, out_ptr,
    ...
    out0 = view0.load(reg_layout)
    out_tail = tail_view.load(reg_layout)
    ttgl.store(out_ptr + M * N + offs, out0)
    ttgl.store(out_ptr + 2 * M * N + offs, out_tail)


@gluon.jit
def tmem_ldst_unaligned_subword_dynamic_linear_subslice_view_kernel(
    ...
    tmem = allocate_tensor_memory(element_ty, [M, 2 * N], layout=parent_layout)
    view0 = tmem.slice(0, N, dim=1)
    view_after = tmem.slice(N, N, dim=1)
    odd_view = tmem.slice(1, N, dim=1)
    tail_view = tmem.slice(N - 1, N, dim=1)
    selected = view0
    if ttgl.load(selector_ptr) != 0:
        selected = odd_view
    ...
    value = ttgl.convert_layout(value, reg_layout)
    view0.store(ttgl.full([M, N], 11, element_ty, layout=reg_layout))
    view_after.store(ttgl.full([M, N], 17, element_ty, layout=reg_layout))
    selected.store(value)
...
@gluon.jit
def tmem_ldst_unaligned_subword_loop_carried_linear_subslice_view_kernel(
    in_ptr, out_ptr, selector_ptr, parent_layout: ttgl.constexpr,
    M: ttgl.constexpr, N: ttgl.constexpr
):
    element_ty: ttgl.constexpr = in_ptr.dtype.element_ty
...
@gluon.jit
def tmem_ld_red_dynamic_linear_subslice_view_kernel(
    in_ptr, out_ptr, red_ptr, selector_ptr, parent_layout: ttgl.constexpr, N: ttgl.constexpr,
    load_variant: ttgl.constexpr, propagate_nan: ttgl.constexpr
):
    M: ttgl.constexpr = 128
    num_warps: ttgl.constexpr = 4
    ...
    view0 = tmem.slice(0, N, dim=1)
    view1 = tmem.slice(N, N, dim=1)
    store_layout: ttgl.constexpr = view0.get_reg_layout()
    view0.store(ttgl.convert_layout(value, store_layout))
    value1 = value + ttgl.full([M, N], 5.0, ttgl.float32, layout=global_layout)
    view1.store(ttgl.convert_layout(value1, store_layout))
...
@gluon.jit
def tmem_ld_red_loop_carried_linear_subslice_view_kernel(
    in_ptr, out_ptr, red_ptr, selector_ptr, parent_layout: ttgl.constexpr, N: ttgl.constexpr,
    load_variant: ttgl.constexpr, propagate_nan: ttgl.constexpr
):
    M: ttgl.constexpr = 128
...
@gluon.jit
def tmem_copy_no_scales_unaligned_subword_linear_subslice_view_kernel(
...
@gluon.jit
def tmem_copy_no_scales_unaligned_subword_loop_carried_linear_subslice_view_kernel(
    in_ptr, out_ptr, selector_ptr, parent_layout: ttgl.constexpr,
    ...
    smem = ttgl.allocate_shared_memory(element_ty, [M, N], layout=smem_layout)
...
@gluon.jit
def tmem_copy_no_scales_dynamic_linear_subslice_view_checked_kernel(in_ptr, out_ptr, selector_ptr,
                                                                    parent_layout: ttgl.constexpr,
                                                                    M: ttgl.constexpr, N: ttgl.constexpr,
                                                                    swizzle: ttgl.constexpr):
    element_ty: ttgl.constexpr = in_ptr.dtype.element_ty
    ...
    offs = offs_m[:, None] * N + offs_n[None, :]
    value = ttgl.load(in_ptr + offs)
    reg_value = ttgl.convert_layout(value, reg_layout)
    view0.store(reg_value + ttgl.full([M, N], 3, element_ty, layout=reg_layout))
    view1.store(reg_value + ttgl.full([M, N], 7, element_ty, layout=reg_layout))
...
@gluon.jit
def tmem_copy_no_scales_warpx2_dynamic_subslice_view_kernel(
    in_ptr,
    out_ptr,
    selector_ptr,
    shared_layout: ttgl.constexpr,
    ...
    view0 = parent.slice(0, N, dim=1)
    view1 = parent.slice(N, N, dim=1)
    tmem = view0
    if ttgl.load(selector_ptr) != 0:
        tmem = view1
    else:
...
@gluon.jit
def tmem_mma_scaled_dynamic_scale_descriptor_view_format_kernel(
    out_ptr,
    selector_ptr,
    M: ttgl.constexpr,
    N: ttgl.constexpr,
    ...
    scale_layout: ttgl.constexpr = TensorMemoryScalesLayout()
    if SCALE_SIDE == "a":
        a_parent0 = allocate_tensor_memory(a_scale.dtype.element_ty, [M, SCALE_K], scale_layout)
        a_parent1 = allocate_tensor_memory(a_scale.dtype.element_ty, [M, SCALE_K], scale_layout)
        a_scale_tmem0 = a_parent0.reshape((M // 2, 2, SCALE_K)).permute([1, 0, 2]).reshape((M, SCALE_K))
        a_scale_tmem1 = a_parent1.reshape((M // 2, 2, SCALE_K)).permute([1, 0, 2]).reshape((M, SCALE_K))
        b_scale_tmem = allocate_tensor_memory(b_scale.dtype.element_ty, [N, SCALE_K], scale_layout)
        scale_reg_layout_m: ttgl.constexpr = a_scale_tmem0.get_reg_layout()
        scale_reg_layout_n: ttgl.constexpr = b_scale_tmem.get_reg_layout()
        scale_offs_k_m = ttgl.arange(0, SCALE_K, layout=ttgl.SliceLayout(0, scale_reg_layout_m))[None, :]
        scale_offs_k_n = ttgl.arange(0, SCALE_K, layout=ttgl.SliceLayout(0, scale_reg_layout_n))[None, :]
        scale_offs_m = ttgl.arange(0, M, layout=ttgl.SliceLayout(1, scale_reg_layout_m))[:, None]
        scale_offs_n = ttgl.arange(0, N, layout=ttgl.SliceLayout(1, scale_reg_layout_n))[:, None]
        a_scale_tmem0.store(ttgl.load(a_scale + scale_offs_m * SCALE_K + scale_offs_k_m))
        a_scale_tmem1.store(ttgl.load(a_scale + scale_offs_m * SCALE_K + scale_offs_k_m))
        b_scale_tmem.store(ttgl.load(b_scale + scale_offs_n * SCALE_K + scale_offs_k_n))
        a_scale_selected = a_scale_tmem0
        if ttgl.load(selector_ptr) != 0:
            a_scale_selected = a_scale_tmem1
        else:
            a_scale_selected = a_scale_tmem0
        a_scale_operand = a_scale_selected
        b_scale_operand = b_scale_tmem
    else:
        a_scale_tmem = allocate_tensor_memory(a_scale.dtype.element_ty, [M, SCALE_K], scale_layout)
        b_parent0 = allocate_tensor_memory(b_scale.dtype.element_ty, [N, SCALE_K], scale_layout)
        b_parent1 = allocate_tensor_memory(b_scale.dtype.element_ty, [N, SCALE_K], scale_layout)
        b_scale_tmem0 = b_parent0.reshape((N // 2, 2, SCALE_K)).permute([1, 0, 2]).reshape((N, SCALE_K))
        b_scale_tmem1 = b_parent1.reshape((N // 2, 2, SCALE_K)).permute([1, 0, 2]).reshape((N, SCALE_K))
        scale_reg_layout_m: ttgl.constexpr = a_scale_tmem.get_reg_layout()
        scale_reg_layout_n: ttgl.constexpr = b_scale_tmem0.get_reg_layout()
        scale_offs_k_m = ttgl.arange(0, SCALE_K, layout=ttgl.SliceLayout(0, scale_reg_layout_m))[None, :]
        scale_offs_k_n = ttgl.arange(0, SCALE_K, layout=ttgl.SliceLayout(0, scale_reg_layout_n))[None, :]
        ...
        scale_offs_n = ttgl.arange(0, N, layout=ttgl.SliceLayout(1, scale_reg_layout_n))[:, None]
        a_scale_tmem.store(ttgl.load(a_scale + scale_offs_m * SCALE_K + scale_offs_k_m))
        b_scale_tmem0.store(ttgl.load(b_scale + scale_offs_n * SCALE_K + scale_offs_k_n))
        b_scale_tmem1.store(ttgl.load(b_scale + scale_offs_n * SCALE_K + scale_offs_k_n))
        b_scale_selected = b_scale_tmem0
        if ttgl.load(selector_ptr) != 0:
            b_scale_selected = b_scale_tmem1
        else:
            b_scale_selected = b_scale_tmem0
        a_scale_operand = a_scale_tmem
        b_scale_operand = b_scale_selected
...
CP_ACTIVE_SUBWORD_LINEAR_SUBSLICE_VIEW_CASES = [
    (
        dtype_name,
        torch_dtype,
        128,
        128,
        32,
        128 * CP_NO_SCALES_SUBWORD_BITWIDTHS[dtype_name] // 256,
    )
...
ACTIVE_SUBWORD_LDST_LINEAR_SUBSLICE_VIEW_CASES = [
    (
        dtype_name,
        torch_dtype,
        bitwidth,
        SUBWORD_LDST_SHAPE_MAP_BY_BITS[bitwidth]["auto"][128],
    )
    for dtype_name, torch_dtype, bitwidth in (
        ("f16", torch.float16, 16),
        ("i8", torch.int8, 8),
    )
]
...
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell")
@pytest.mark.parametrize("dtype_name,torch_dtype,bitwidth,expected_shape", ACTIVE_SUBWORD_LDST_LINEAR_SUBSLICE_VIEW_CASES)
@pytest.mark.parametrize("selector", (0, 1))
def test_tmem_runtime_matrix_ldst_dynamic_linear_subslice_view_subword(
    dtype_name, torch_dtype, bitwidth, expected_shape, selector
):
    m = 128
    n = 128
    layout = _make_tmem_linear_layout(m, 2 * n)
    ...
    out = torch.empty((2, m, n), dtype=torch_dtype, device="cuda")
    selector_tensor = torch.tensor(selector, dtype=torch.int32, device="cuda")
    ...
    plus3 = (base + 3).to(torch_dtype)
    plus7 = (base + 7).to(torch_dtype)
    expected0 = inp if selector == 0 else plus3
    expected1 = inp if selector == 1 else plus7
    torch.testing.assert_close(out[0], expected0, atol=0, rtol=0)
    torch.testing.assert_close(out[1], expected1, atol=0, rtol=0)
...
def test_tmem_runtime_matrix_ldst_loop_carried_linear_subslice_view_subword(
    dtype_name, torch_dtype, bitwidth, expected_shape, selector, selects_view1
):
    m = 128
    n = 128
    layout = _make_tmem_linear_layout(m, 2 * n)
    ...
    plus3 = (base + 3).to(torch_dtype)
    plus7 = (base + 7).to(torch_dtype)
    expected_selected_before = plus7 if selects_view1 else plus3
    expected0 = plus3 if selects_view1 else inp
    expected1 = inp if selects_view1 else plus7
    torch.testing.assert_close(out[0], expected_selected_before, atol=0, rtol=0)
    torch.testing.assert_close(out[1], expected0, atol=0, rtol=0)
    torch.testing.assert_close(out[2], expected1, atol=0, rtol=0)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("dynamic", 0, False, id="dynamic-view0"),
        pytest.param("dynamic", 1, True, id="dynamic-odd"),
        pytest.param("loop_carried", 2, False, id="loop-view0"),
        pytest.param("loop_carried", 0, True, id="loop-odd"),
    ],
)
def test_tmem_runtime_matrix_ldst_unaligned_subword_selected_view_roundtrip(
    dtype_name, torch_dtype, mode, selector, selects_odd
):
    m = 128
    n = 128
    layout = _make_tmem_linear_layout(m, 2 * n)
    ...
    kernel = (
        tmem_ldst_unaligned_subword_dynamic_linear_subslice_view_kernel
        if mode == "dynamic"
        else tmem_ldst_unaligned_subword_loop_carried_linear_subslice_view_kernel
    )
    compiled = kernel[(1, )](inp, out, selector_tensor, layout, m, n, num_warps=4)
...
@pytest.mark.skipif(not is_blackwell_ultra(), reason="Requires Blackwell Ultra")
@pytest.mark.parametrize("selector", (0, 1))
def test_tmem_runtime_matrix_ld_red_dynamic_linear_subslice_view(selector):
    M = N = 128
    layout = _make_tmem_linear_layout(M, 2 * N)
    inp = torch.randn(M, N, dtype=torch.float32, device="cuda")
    out = torch.empty_like(inp)
    ...
    compiled = tmem_ld_red_dynamic_linear_subslice_view_kernel[(1, )](
        inp,
        out,
        red,
        selector_tensor,
    ...
    expected = inp + 5 if selector else inp
    torch.testing.assert_close(out, expected, atol=0, rtol=0)
    torch.testing.assert_close(red, torch.max(expected, dim=1).values, atol=0, rtol=0)
    _assert_ld_red_opcode_pairs(
        compiled,
...
def test_tmem_runtime_matrix_ld_red_loop_carried_linear_subslice_view(selector, selects_view1):
    M = N = 128
    layout = _make_tmem_linear_layout(M, 2 * N)
    inp = torch.randn(M, N, dtype=torch.float32, device="cuda")
    out = torch.empty_like(inp)
    ...
    compiled = tmem_ld_red_loop_carried_linear_subslice_view_kernel[(1, )](
        inp,
        out,
        red,
        selector_tensor,
    ...
    expected = inp + 5 if selects_view1 else inp
    torch.testing.assert_close(out, expected, atol=0, rtol=0)
    torch.testing.assert_close(red, torch.max(expected, dim=1).values, atol=0, rtol=0)
    _assert_ld_red_opcode_pairs(
        compiled,
...
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell")
@pytest.mark.parametrize("selector", (0, 1))
def test_tmem_runtime_matrix_cp_no_scales_dynamic_linear_subslice_view(selector):
    M = 128
    N = 128
    swizzle = 32
    inp = torch.arange(M * N, device="cuda", dtype=torch.float32).reshape(M, N)
    ...
    parent_layout = _make_tmem_linear_layout(M, 2 * N)
    compiled = tmem_copy_no_scales_dynamic_linear_subslice_view_kernel[(1, )](
        inp,
        out,
        selector_tensor,
        parent_layout,
...
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell")
@pytest.mark.parametrize("dtype_name,torch_dtype,M,N,swizzle,expected_count", CP_ACTIVE_SUBWORD_LINEAR_SUBSLICE_VIEW_CASES)
@pytest.mark.parametrize("selector", (0, 1))
def test_tmem_runtime_matrix_cp_no_scales_dynamic_linear_subslice_view_subword(
    dtype_name, torch_dtype, M, N, swizzle, expected_count, selector
):
    base = torch.arange(M * N, device="cuda", dtype=torch.int32).reshape(M, N) % 16
    inp = base.to(torch_dtype)
    out = torch.empty((2, M, N), device="cuda", dtype=torch_dtype)
    selector_tensor = torch.tensor(selector, dtype=torch.int32, device="cuda")

    parent_layout = _make_tmem_linear_layout(M, 2 * N)
    compiled = tmem_copy_no_scales_dynamic_linear_subslice_view_checked_kernel[(1, )](
        inp,
        out,
        selector_tensor,
        parent_layout,
...
def test_tmem_runtime_matrix_cp_no_scales_loop_carried_linear_subslice_view_subword(
    dtype_name, torch_dtype, M, N, swizzle, expected_count, selector, selects_view1
):
    base = torch.arange(M * N, device="cuda", dtype=torch.int32).reshape(M, N) % 16
    inp = base.to(torch_dtype)
    out = torch.empty((2, M, N), device="cuda", dtype=torch_dtype)
...
def test_tmem_runtime_matrix_cp_no_scales_warpx2_dynamic_subslice_view_positive(
    selector,
    family,
    parent_layout_fn,
    expected_fn,
    ...
    compiled = tmem_copy_no_scales_warpx2_dynamic_subslice_view_kernel[(1, )](
        inp, out, selector_tensor, shared_layout, parent_layout, num_warps=4
    )
...
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell")
@pytest.mark.parametrize("scale_side", ["a", "b"])
def test_tmem_runtime_matrix_mma_scaled_dynamic_scale_descriptor_view(scale_side):
    m = n = k = 128
    a_format = b_format = "mxfp8"
    layout = _make_tmem_linear_layout(m, n)
    vec_size = 32
    ...
    compiled = tmem_mma_scaled_dynamic_scale_descriptor_view_format_kernel[(1, )](
        out,
        selector,
        m,
        n,
