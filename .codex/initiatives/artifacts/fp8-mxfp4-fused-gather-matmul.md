...
- [ ] Do not spend more bounded tuning effort on `ws_split_sf`; if more target-bucket gain is required beyond the packed-store win, the next move is either a larger manual epilogue fragment rewrite / compiler-aware layout work or a larger WS producer-pipeline rewrite with the user
