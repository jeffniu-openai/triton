...
- Latest: 2026-04-17 19:25 UTC Phase C support-bearing no-scales two-CTA
  ...
  the canonical `warpx2` source layout with shared block basis `[[128, 0]]`
  and emits a cluster async-shared fence before the backend copy. The former
  two-CTA `01_23` dense-source clean negatives now pass runtime/opcode
  coverage for f32/i32. Dense two-CTA `02_13` rematerializes far enough to hit
  the true high source-column preservation diagnostic and remains negative.
  Validation: `make -j8`, focused two-CTA dense selector (`4 passed`), full
  `cp_no_scales_warpx2` (`79 passed`), split-4 groups (`20/20/20/19`
  passed), Python compile. Rebaseline: `reports_clean_unsupported`
  `115/1592`; combined clean-negative/error `165/1592`. Next concrete slice:
  investigate `warpx2` subword/packed-lane copies or keep them as typed
  storage boundaries if no exact packed-lane staging schedule exists.
...
- Current plain MMAv5 TMEM-LHS subview K-depth checkpoint, 2026-04-14 16:39 UTC: TMEM-LHS `memdesc_subslice` descriptor-view coverage now includes `K=128`. `MMA_LHS_SUBSLICE_NK_CASES` spans every supported plain kind, legacy/canonical accumulator layouts, `N in {64,128,256}`, and `K in {32,64,128}` for both zero-accumulator and `use_acc=True` paths through a `[128,2*K]` LHS parent. This adds `60` executable cases and closes the obvious plain-MMAv5 K128 descriptor-view gap left after root/index/accumulator-subview/TMA work. Runtime-matrix collection is now `8890` tests: `cp=677`, `mma=2703`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `8439 passed, 451 skipped`. Validation: `make -j8`; py-compile; full-file collect `8890`; `mma_lhs_subslice_view_plain_kinds` collect `180/8890`; four-GPU split execution passed all `180` selected cases (`45` per group; slowest `86.02s`).

- Current one-CTA plain MMAv5 indexed-accumulator K-depth checkpoint, 2026-04-14 16:36 UTC: one-CTA accumulator `memdesc_index` descriptor-view coverage now includes `K=128`. `MMA_INDEXED_ACC_CASES` spans every supported plain kind, `N in {64,128,256}`, `K in {32,64,128}`, and both `use_acc` modes through legacy parents, fitting canonical linear parents, and canonical `linear_unit_parent` rows for `N=256`. This adds `60` executable cases and keeps the live `[2,128,256]` canonical-linear parent as a resource boundary while using `[1,128,256].index(0)` for the positive construction. Runtime-matrix collection is now `8830` tests: `cp=677`, `mma=2643`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `8379 passed, 451 skipped`. Validation: `make -j8`; py-compile; full-file collect `8830`; focused one-CTA `mma_indexed_acc_view` collect `180/8830`; four-GPU split execution passed all `180` selected cases (`45` per group; slowest `96.47s`).

- Current TMA-fed two-CTA plain MMAv5 K-depth checkpoint, 2026-04-14 16:32 UTC: descriptor-fed two-CTA `tcgen05.mma` coverage now includes `K=128`. Non-TF32 TMA positives span f16, bf16, f8e5m2, and f8e4m3 at `N in {64,128,256}`, `K in {32,64,128}`, legacy/canonical two-CTA accumulators, and both accumulator modes. TF32 TMA rows now include `K=128` for the default clean-negative transposed-shared descriptor route and for the positive B-transposed descriptor route with and without accumulator add. Runtime-matrix collection is now `8770` tests: `cp=677`, `mma=2583`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `8319 passed, 451 skipped`. Validation: `make -j8`; py-compile; full-file collect `8770`; exact TMA-fed collect `198`; four-GPU split execution passed all `198` selected cases (`50`, `50`, `50`, `48`; slowest `60.96s`).

- Current two-CTA plain MMAv5 accumulator-subview K-depth checkpoint, 2026-04-14 16:29 UTC: two-CTA accumulator `memdesc_subslice` descriptor-view coverage now includes `K=128`. `MMA_TWOCTA_ACC_SUBSLICE_CASES` spans every supported plain kind, `N in {64,128,256}`, `K in {32,64,128}`, both slice starts, and both `use_acc` modes through linear two-CTA `[256,2*N]` parents. This adds `60` executable cases and completes the direct two-CTA plain root/index/subslice K128 group; TMA-fed matrices and scaled-MMAv5 K-depth remain future slices. Runtime-matrix collection is now `8704` tests: `cp=677`, `mma=2517`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `8253 passed, 451 skipped`. Validation: `make -j8`; py-compile; full-file collect `8704`; `mma_twocta_acc_subslice_view_plain_kinds` collect `180/8704`; four-GPU split execution passed all `180` selected cases (`45` per group; slowest `57.05s`).

- Current two-CTA plain MMAv5 indexed-accumulator K-depth checkpoint, 2026-04-14 16:27 UTC: two-CTA accumulator `memdesc_index` descriptor-view coverage now includes `K=128`. `MMA_TWOCTA_INDEXED_ACC_CASES` spans every supported plain kind, `N in {64,128,256}`, `K in {32,64,128}`, and both `use_acc` modes through legacy parents, canonical linear parents for fitting `N`, and canonical `linear_unit_parent` rows for `N=256`. This adds `60` executable cases and is scoped to two-CTA indexed accumulator views only; two-CTA accumulator subviews, TMA-fed matrices, and scaled-MMAv5 K-depth remain future slices. Runtime-matrix collection is now `8644` tests: `cp=677`, `mma=2457`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `8193 passed, 451 skipped`. Validation: `make -j8`; py-compile; full-file collect `8644`; `mma_twocta_indexed_acc_view` collect `180/8644`; four-GPU split execution passed all `180` selected cases (`45` per group; slowest `111.01s`).

- Current two-CTA plain MMAv5 K-depth checkpoint, 2026-04-14 16:22 UTC: direct two-CTA plain root `tcgen05.mma` coverage now includes `K=128`. `MMA_TWOCTA_PLAIN_KIND_CASES` spans every supported plain kind, legacy/canonical two-CTA TMEM-linear accumulator layouts, `N in {64,128,256}`, and `K in {32,64,128}`; both `test_tmem_runtime_matrix_mma_twocta_plain_kinds` and `test_tmem_runtime_matrix_mma_twocta_plain_kinds_use_acc` consume this table. This adds `60` executable cases and is scoped to direct two-CTA root accumulators only; two-CTA indexed-accumulator views, two-CTA accumulator subviews, TMA-fed matrices, and scaled-MMAv5 K-depth remain future slices. Runtime-matrix collection is now `8584` tests: `cp=677`, `mma=2397`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `8133 passed, 451 skipped`. Validation: `make -j8`; py-compile; full-file collect `8584`; `mma_twocta_plain_kinds` collect `180/8584`; four-GPU split execution passed all `180` selected cases (`45` per group); broader `-k 'mma and not cp'` collect selected `2632/8584` because it also matches MMAv5-flavored `ld/st` parameter ids.

- Current copy `warpx2` slice-plus-index checkpoint, 2026-04-14 16:14 UTC: no-scales `tcgen05.cp` now supports leading-unit `ttg.memdesc_subslice` views followed by `ttg.memdesc_index` for the reachable public `warpx2` layouts. `tryMakeLeadingUnitSubviewLayout` in `TensorMemoryUtils.cpp` handles same-rank layout subviews that slice leading logical dimensions to unit length, turns consumed logical offset bits into query-origin deltas, squeezes the consumed dimensions, and erases only exact inactive physical bases for those consumed dimensions so unrelated zero bases keep broadcast semantics. Runtime coverage includes single-CTA `warpx2::{01_23,02_13}` positives and two-CTA `warpx2::01_23` positives through `[2,M,4].slice(parent_index,1,dim=0).index(0)`, plus clean-negative two-CTA `warpx2::02_13`; `ops.mlir` now expects the squeezed active-view encoding. Runtime-matrix collection at this checkpoint was `8524` tests: `cp=677`, `mma=2337`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`. Validation: `make -j8`; py-compile; full collect `8524`; `slice_index_view=16`, `warpx2=83`, `cp_no_scales=310`; split-4 execution passed `slice_index_view` and `warpx2`, CP passed/skipped `300 passed, 10 skipped`; lit `invalid.mlir`, `ops.mlir`, and `tritongpu_to_llvm_blackwell.mlir` passed.

- Current copy `warpx2` indexed-view checkpoint, 2026-04-14 15:54 UTC: no-scales `tcgen05.cp` now has explicit generic `ttg.memdesc_index` descriptor-view coverage for the reachable public warp2x layouts. Single-CTA `warpx2::{01_23,02_13}.64x128b` copies run through `[2,128,4].index(parent_index)` with `parent_index in {0,1}`, `f32`/`i32` payloads, exact copy and commit opcodes, and runtime oracles. Two-CTA `warpx2::01_23.64x128b` now has the analogous `[2,256,4].index(parent_index)` positive coverage with exact multicast commit checks and no `cta_group::1` leakage. Two-CTA `warpx2::02_13.64x128b` indexed views are pinned as clean unsupported with the same high-source-column-bit diagnostic as the direct and subview forms. This replaces the older scratch note that direct lifted `warpx2` indexing was not a current positive target; direct outer `memdesc_index` is now covered, while slice-plus-index and true tensor-memory-scales `warpx2` remain separate frontiers. Runtime-matrix collection is now `8508` tests: `cp=661`, `mma=2337`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `8057 passed, 451 skipped`. Validation: `make -j8`; py-compile; full-file collect `8508`; `warpx2` collect `67/8508`; CP collect `661/8508`; four-GPU `warpx2` selector passed all `67` cases (`17`, `17`, `17`, and `16`; slowest `67.26s`); four-GPU CP selector passed/skipped `651 passed, 10 skipped` (`156 passed/10 skipped`, `166 passed`, `166 passed`, and `163 passed`; slowest `225.83s`); `git diff --check` passed.
...
- Current scales `ld/st` descriptor-view checkpoint, 2026-04-14 15:32 UTC: default/no-CGA tensor-memory scales allocations remain positive through the TMEM reshape/view algebra, and the follow-up CGA probe now blocks the unsound two-CTA int8 type-only fallback that previously compiled some descriptor views into wrong runtime data. Scales view/query analysis now derives the `twoCTAs` bit from `TensorMemoryScalesLayout` CGA layout instead of assuming one CTA globally. Default/no-CGA positives still cover `(128,32)`, `(128,64)`, and `(256,64)` reshaped/permuted/reshaped into canonical `tensor_memory_linear` views with exact root scales and view-linear opcode streams. Non-default-CGA/two-CTA int8 descriptor views with support/broadcast bases are pinned as clean unsupported for explicit `32x32b` descriptor-view `ld/st`: `(128,64)`, `(256,32)`, and `(256,64)` now fail before the standalone query-type fallback can erase the physical projection. Runtime-matrix collection is now `8230` tests: `cp=645`, `mma=2075`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`; bucketed evidence aggregates to `7779 passed, 451 skipped`. Do not infer support for scales multibuffering, true scales `warpx2`, low-shape row-anchor cases, or positive two-CTA/CGA scales descriptor views from this slice; a future positive must preserve the exact physical/support projection rather than canonicalizing by type alone.
...
- The new positive matrix covers dense no-scales `tcgen05.cp.cta_group::2.128x256b` through a canonical TMEM-linear two-CTA parent `[M, 2*N]` sliced to a `[M, N]` active column view. Axes are `f32`/`i32`, `M=256`, `N in {128,256}`, and swizzle byte widths `{32,64,128}`.
- The helper mirrors the existing two-CTA copy synchronization path: two-CTA CGA shared layout, `fence_async_shared(cluster=True)`, multicast mbarrier commit, exact `cta_group::2` copy opcodes, no `cta_group::1` copies, and generic `ttg.memdesc_subslice` rather than `ttng.tmem_subslice`.
- This closes a reachable descriptor-view gap in dense copy coverage. It does not unpark no-scales two-CTA `warpx2::02_13` or true tensor-memory-scales `warpx2`; those still need a real descriptor/address/staging model.
...
- Validation completed: `make -j8`; `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; no-PYTHONPATH focused collect selected `12/8203`; no-PYTHONPATH CP collect selected `624/8203`; no-PYTHONPATH full-file collect reported `8203`; focused selector passed all `12` cases across split-4 on four GPUs (`3` per group; group times `5.31s`, `5.54s`, `5.31s`, and `5.74s`); full CP selector passed/skipped `614 passed, 10 skipped` across split-4 (`146 passed/10 skipped`, `156 passed`, `156 passed`, and `156 passed`; slowest `216.93s`); `git diff --check` passed.
- Tooling note: `apply_patch` still fails in this environment with `No such file or directory`; this source/docs slice used exact scripted replacements instead.
- Next: commit/push this bounded copy descriptor-view checkpoint, then continue staged ISA saturation in another exact non-parked family. Good next candidates are a two-CTA dense indexed-copy descriptor-view probe, another `ld/st` descriptor-view gap, or remaining MMAv5/scaled-MMAv5 parity that avoids the parked scale/warpx2 frontiers.
...
- Coverage spans direct and descriptor-chain `128x{64,128,256}` layouts over every public `ld/st` variant for pure row, pure column, and mixed non-identity row+column permutations. Identity/identity i32 remains covered by the broad identity/mixed i32 tests rather than duplicated here.
- This completes the staged broad row/column i32 parity work after the pure row/column slice. The implementation is test-only: no compiler/lowering source changed.
- Current runtime-matrix collection is `4727` tests: `cp=322`, `mma=730`, splitn/misc `=252`, `ld_red=811`, and `ldst=2612`; current bucketed evidence aggregates to `4281 passed, 446 skipped`.
- Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; `make -j8`; no-PYTHONPATH focused i32 mixed collect selected `270/4727`; no-PYTHONPATH `ldst` collect selected `2612/4727`; focused mixed row/column i32 selector passed all `270` cases across split-16 on four GPUs (group sizes `17` for groups 1-15 and `15` for group 16; group times `95.87s`, `90.37s`, `98.35s`, `51.10s`, `91.40s`, `95.88s`, `98.71s`, `9.62s`, `425.76s`, `417.78s`, `423.54s`, `212.60s`, `422.09s`, `430.72s`, `435.20s`, and `4.17s`). Full `ldst` bucket was not rerun because no shared lowering changed.

## 2026-04-14 05:57 UTC: ld/st broad pure row/column i32 parity

- `LDST_ROWCOL_PERMUTED_CASES` now parameterizes over f32 for the full row/column cross-product and adds i32 for the pure-row and pure-column subset (`row=identity xor col=identity`).
- Coverage spans direct and descriptor-chain `128x{64,128,256}` layouts over every public `ld/st` variant for pure row and pure column permutations; mixed non-identity row+column i32 remains intentionally deferred as a larger duration-aware slice.
...
- Current runtime-matrix collection is `4457` tests: `cp=322`, `mma=730`, splitn/misc `=252`, `ld_red=811`, and `ldst=2342`; current bucketed evidence aggregates to `4011 passed, 446 skipped`.
- Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; `make -j8`; no-PYTHONPATH focused i32 collect selected `180/4457`; no-PYTHONPATH `ldst` collect selected `2342/4457`; focused pure row/column i32 selector passed all `180` cases across split-8 (`23`, `23`, `23`, `23`, `23`, `23`, `23`, and `19` cases; group times `122.20s`, `127.13s`, `116.86s`, `159.46s`, `567.37s`, `560.65s`, `510.40s`, and `489.88s`). Full `ldst` bucket was not rerun because no shared lowering changed.
...
- Coverage spans direct and descriptor-chain `128x{64,128,256}` layouts for identity plus `rotate1`, `even_odd`, and `reverse` diagonal row/column permutations over every public `ld/st` variant.
- The affected direct/descriptor tests now seed int32 data, cast to f32/i32, and use dtype-preserving descriptor-chain `inp + 3` expectations while retaining exact opcode-shape assertions.
- Current runtime-matrix collection is `4217` tests: `cp=322`, `mma=730`, splitn/misc `=252`, `ld_red=811`, and `ldst=2102`; current bucketed evidence aggregates to `3771 passed, 446 skipped`.
- Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; `make -j8`; no-PYTHONPATH focused collect selected `240/4217`; no-PYTHONPATH `ldst` collect selected `2102/4217`; initial split-4 focused run passed groups 1 and 2 but groups 3 and 4 timed out in descriptor-heavy shards; split-8 focused rerun passed all `240` cases (`30` per group; group times `4.33s`, `159.08s`, `106.34s`, `159.25s`, `535.99s`, `754.10s`, `376.30s`, and `407.55s`). Full `ldst` bucket was not rerun because no shared lowering changed.
...
- The new matrix covers `block_two_ctas` and MMAv5-like two-CTA TMEM-linear layouts at `N in {64, 128, 256}`, both direct TMEM access and the supported descriptor-chain path, and every public `ld/st` variant.
- Existing broad two-CTA f32 direct/descriptor coverage remains in the original tests; this follows the prior single-CTA i32 parity pattern without rewriting established f32 nodeids.
- Current runtime-matrix collection is `3789` tests: `cp=322`, `mma=502`, splitn/misc `=252`, `ld_red=811`, and `ldst=1902`; current bucketed evidence aggregates to `3343 passed, 446 skipped`.
- Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `make -j8`; no-PYTHONPATH focused collect selected `60/3789`; no-PYTHONPATH `ldst` collect selected `1902/3789`; focused two-CTA i32 broad selector passed `60` across four GPUs (`15` each), with group times about `74.63s`, `21.09s`, `304.62s`, and `152.89s`; `git diff --check`.
...
- The slice covers direct access and supported descriptor-chain views over all public `ld/st` variants, pinning minimal atom shapes (`32x32b.x32`, `16x64b.x16`, `16x128b.x8`, `16x256b.x4`) and the `twoCTAs` TTGIR marker.
- Current runtime-matrix collection is `3371` tests: `cp=322`, `mma=324`, splitn/misc `=252`, `ld_red=771`, and `ldst=1702`; current bucketed evidence aggregates to `2925 passed, 446 skipped`.
- Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; `make -j8`; no-PYTHONPATH focused collect selected `20/3371`; focused selector passed `20` cases across four GPUs (`5` each), slowest split about `1:35`.
- Remaining long-term work: continue bounded `ld/st` row/column cross-product and view-chain slices before a broad `ldst` runner refresh.
...
- Before the later scaled-MMAv5 subview expansion, runtime-matrix bucket totals were `cp=318`, `mma=301`, splitn/misc `=252`, `ld_red=771`, `ldst=1642`; bucketed evidence aggregated to `2838 passed, 446 skipped`.
...
- Latest scaled-MMAv5 two-CTA accumulator-subview `block_n=256` probe, 2026-04-13 after the tight MMA validation: representative `mxfp8/mxfp8` attempts with `block_n=256`, `parent_n=512`, `slice_start in {0,256}`, and both multicast modes fail launch metadata with tensor-memory OOR (`Required: 524`, hardware limit `512`). Trying `parent_n=384`, `slice_start in {0,128}` is rejected by `_make_tmem_linear_layout_mmav5_twocta` because the helper currently requires power-of-two `N`. Do not treat this as a simple missing positive in `test_tmem_runtime_matrix_mma_scaled_twocta_acc_subslice_view_format_matrix`; current legal offset-subview coverage remains `block_n=128`, `parent_n=256`, `slice_start in {0,128}`.
- Latest broad direct MMA/scaled-MMA runtime-matrix validation checkpoint, 2026-04-13 after the plain LHS `N` expansion: `python/test/gluon/test_tmem_runtime_matrix.py -k 'test_tmem_runtime_matrix_mma'` collected the pure MMA/scaled-MMA family (`301` cases) and passed across four GPU split groups (`76`, `76`, `76`, `73`). Do not use the older `-k 'mma and not cp'` phrase for pure MMA validation: collect-only showed it also selects `ldst_*mmav5*` cases because `mmav5` contains `mma`, which explains older broad-selector imbalance/timeouts.
- Superseded broad `tcgen05.cp` runtime-matrix validation checkpoint, 2026-04-13 after the scaled-copy multicast expansion: `python/test/gluon/test_tmem_runtime_matrix.py -k 'cp'` passed across four GPU split groups with aggregate `307 passed, 5 skipped` (`73 passed, 5 skipped`; `78 passed`; `78 passed`; `78 passed`). This supersedes older `227 passed, 5 skipped` broad-copy evidence; the increase is expected from adding all-format multicast scaled-copy cases.
...
- Latest TMA-fed two-CTA TF32 clean-negative checkpoint, 2026-04-13 after the direct-i8 clean-negative expansion: `test_tmem_runtime_matrix_mma_twocta_tma_tf32_reports_clean_shared_transpose_error` now parameterizes `block_n` over `128` and `256` for both legacy and canonical TMEM-linear accumulator layouts. The default `[K, N]` B TMA descriptor path now has the same shape coverage as the supported `[N, K]` descriptor plus shared-permute route. Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `make` no-op success; exact expanded negative nodeid across four GPU split groups (`4 passed` aggregate); nearby `-k 'tma_tf32'` selector passed all eight selected positive plus negative cases across the four groups.
- Latest direct-i8 MMAv5 clean-negative checkpoint, 2026-04-13 after the TMA TF32 descriptor expansion: `test_tmem_runtime_matrix_mma_twocta_i8_reports_clean_error` now parameterizes `block_n` over `128` and `256` for both legacy and canonical TMEM-linear two-CTA accumulator layouts. This aligns the clean unsupported direct-i8 boundary with the supported two-CTA plain-kind `blockN` matrix. Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `make` no-op success; exact expanded nodeid across four GPU split groups (`4 passed` aggregate); nearby `-k 'i8_reports_clean_error'` selector passed all six selected one-CTA plus two-CTA cases across groups 1-3, with group 4 selecting no tests.
- Latest TMA-fed MMAv5 TF32 B-transposed descriptor checkpoint, 2026-04-13 after the scales `warpx2` scan: `python/test/gluon/test_tmem_runtime_matrix.py::test_tmem_runtime_matrix_mma_twocta_tma_tf32_b_transposed_descriptor` now parameterizes `block_n` over `128` and `256` for both legacy and canonical TMEM-linear accumulator layouts. Scratch probes for the new `block_n=256` cases passed on GPUs 0/1 and emitted four matching PTX/LLIR `tcgen05.mma.cta_group::2.kind::tf32` instructions for each accumulator layout. Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `make` no-op success; exact expanded nodeid across four GPU split groups (`4 passed` aggregate); nearby `-k 'tma_tf32'` selector passed all six selected cases across groups 1-3, with group 4 selecting no tests because the split was exhausted. The clean negative for the default `[K, N]` B TMA descriptor remains unchanged.
...
- Copy-planner debug checkpoint, 2026-04-13 13:43 UTC: temporary verifier/lowering instrumentation was added, rebuilt, used for exact copy nodeids, then removed and rebuilt cleanly. The two unsupported copy frontiers are verifier rejections: two-CTA `warpx2::02_13` tries direct-seed `64x4`, descriptor `64x4`, and fallback descriptor `32x4` plans, but direct seed is deliberately unavailable for `256x4` and no descriptor candidate is MMASMEM-representable; the scales `warpx2_candidate` maps to `warpx4.32x128b` and has one descriptor candidate that is not representable. Known-good two-CTA `01_23` succeeds through a `32x4` descriptor candidate, while single-CTA `02_13` succeeds only because `128x4` direct seed is accepted. Do not promote `256x4` direct seed based on this: corrected direct-PTX evidence shows it duplicates source columns rather than realizing the full two-CTA `02_13` oracle.
...
- Latest `ld.red` explicit-variant checkpoint, 2026-04-13 13:35 UTC: explicit compatible reduction-load layouts now cover all legal modifier combinations (`abs` false/true and `PropagateNan.NONE/ALL`) for both `min` and `max`. `tmem_ld_red_explicit_layout_kernel` forwards those modifiers into `tmem.load_{min,max}`, the tests inject NaNs for `PropagateNan.ALL`, validate PyTorch-equivalent reduction semantics, and still pin canonical `tcgen05.ld.red.sync.aligned.32x32b.x128` lowering for identity plus the compatible non-identity layouts (`tile_permuted`, `col_reverse`, `row_reverse`, `rowcol_rotate_reverse`) across `auto`, `32x32b`, `16x32bx2`, and `32x32b_splitn`. Focused validation passed `163` selected cases across four GPU split groups (`41`, `41`, `41`, `40`) after py-compile and no-op rebuild. The older broad `-k ld_red` green evidence remains `523 passed`; future broad local reruns should still use duration data/finer groups because cold equal-count splits can be imbalanced.
...
## Latest: 2026-04-14 13:10 UTC TMA-fed two-CTA TF32 K-width coverage
...
## Latest: 2026-04-14 13:11 UTC copy warpx2 subword clean-negative coverage

- Runtime-matrix collection is now `7891` tests with bucket totals `cp=588`, `mma=1975`, splitn/misc `=499`, `ld_red=1920`, and `ldst=2909`; current bucketed evidence aggregates to `7440 passed, 451 skipped`.
- Latest coverage slice adds `CP_NO_SCALES_WARPX2_SUBWORD_UNSUPPORTED_CASES`, covering f16/bf16 source payloads for the single-CTA `warpx2::01_23`, single-CTA `warpx2::02_13`, two-CTA `warpx2::01_23`, and two-CTA `warpx2::02_13` descriptor families.
- All rows assert the clean 32-bit source-element contract (`Source element type should be 32-bit.`) and absence of PassManager/assertion noise. This does not change the supported positive set or unpark true scales `warpx2` / no-scales two-CTA `02_13`.
- Validation for the latest slice: one-off f16/bf16 probes for all four family/CTA combinations; `make -j8`; `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; no-PYTHONPATH subword collect selected `8/7891`; no-PYTHONPATH full `warpx2` collect selected `29/7891`; no-PYTHONPATH CP collect selected `588/7891`; no-PYTHONPATH full-file collect reported `7891`; full `warpx2` selector passed all `29` cases across split-4 (`8`, `8`, `8`, and `5`; group times `8.57s`, `4.87s`, `5.02s`, and `4.58s`); `git diff --check` passed.

## 2026-04-14 16:14 UTC: copy warpx2 slice-plus-index descriptor-view support

- Fixed leading-unit TMEM `memdesc_subslice` view algebra for no-scales tensor-memory-linear descriptor chains. `tryMakeLeadingUnitSubviewLayout` recognizes layout-rank subviews where leading logical dimensions are sliced to size `1`, converts selected leading logical bits into the query origin, squeezes the consumed logical dimensions, and erases only the exact inactive physical bases associated with those consumed bits. This preserves unrelated row/column zero bases as semantic broadcast axes instead of trimming them as dead state.
- Wired the helper into TMEM subslice query inference, subslice op encoding inference, standalone subslice encoding inference, and pointer base-offset lowering. The lowering path now computes leading-unit subview offsets from ld/st query-origin deltas instead of pseudoinverting non-injective parent layouts.
- Added runtime-matrix coverage for `slice(..., dim=0).index(0)` over `warpx2` parents:
  - single-CTA `warpx2::01_23` and `warpx2::02_13` from `[2,128,4]` parents, `parent_index in {0,1}`, `f32`/`i32`;
  - two-CTA `warpx2::01_23` from `[2,256,4]` parents, `parent_index in {0,1}`, `f32`/`i32`;
  - two-CTA `warpx2::02_13` slice-plus-index stays a clean high-source-column-bit unsupported case.
- Updated `test/TritonNvidiaGPU/ops.mlir` for the new leading-unit subslice contract: a `1x64x128` slice from `#tmem_linear_rank3` now carries the squeezed `#tmem_linear_half_rows` active-view encoding instead of preserving the old rank-3 physical view.
- Current runtime-matrix collection is `8524` tests: `cp=677`, `mma=2337`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2985`.
