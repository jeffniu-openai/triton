...
- Latest scaled-MMAv5 copy-helper `block_k=256` checkpoint, 2026-04-13 after the direct-root `K=256` expansion: `test_tmem_runtime_matrix_cp_scales_warpx4_via_scaled_mma_copy_matrix` now parameterizes `block_k` over `128` and `256` in addition to every current scaled format pair, `block_n in {128, 256}`, `num_ctas in {1,2}`, and legacy/canonical accumulator layouts. A scratch four-GPU probe confirmed the forty new `block_k=256` combinations match the dequantized reference. Exact scale-copy count is now `(1 + block_n // 128) * (block_k // 128) * (32 // vec_size)`, and exact scaled-MMA count is `(block_k // 128) * base_count`. Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; `make -j8` no-op success; exact expanded nodeid passed `80` cases across four GPU split groups (`20` each); nearby `-k 'cp_scales_warpx4_via_scaled_mma'` passed `112` cases (`28` each); broad `-k 'cp'` passed `227` with `5` skips (`53 passed, 5 skipped`; `58`; `58`; `58`).
...
- Latest broad direct MMA/scaled-MMA runtime-matrix validation checkpoint, 2026-04-13 after the scaled root `K=256` expansion: `python/test/gluon/test_tmem_runtime_matrix.py -k 'test_tmem_runtime_matrix_mma'` collected the pure MMA/scaled-MMA family (`263` cases) and passed across four GPU split groups (`66`, `66`, `66`, `65`). Do not use the older `-k 'mma and not cp'` phrase for pure MMA validation: collect-only showed it also selects `ldst_*mmav5*` cases because `mmav5` contains `mma`, which explains older broad-selector imbalance/timeouts.
- Latest broad `tcgen05.cp` runtime-matrix validation checkpoint, 2026-04-13 after the scaled-copy `block_k=256` expansion: `python/test/gluon/test_tmem_runtime_matrix.py -k 'cp'` passed across four GPU split groups with aggregate `227 passed, 5 skipped` (`53 passed, 5 skipped`; `58 passed`; `58 passed`; `58 passed`). This supersedes older `187 passed, 5 skipped` broad-copy evidence; the increase is expected from adding all-format `block_k=256` scaled-copy cases.
