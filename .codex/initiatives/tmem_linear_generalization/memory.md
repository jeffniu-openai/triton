...
- Latest broad direct MMA/scaled-MMA runtime-matrix validation checkpoint, 2026-04-13 after the scaled LHS-subview `N/K` expansion: `python/test/gluon/test_tmem_runtime_matrix.py -k 'test_tmem_runtime_matrix_mma'` collected the pure MMA/scaled-MMA family (`287` cases) and passed across four GPU split groups (`72`, `72`, `72`, `71`). Do not use the older `-k 'mma and not cp'` phrase for pure MMA validation: collect-only showed it also selects `ldst_*mmav5*` cases because `mmav5` contains `mma`, which explains older broad-selector imbalance/timeouts.
