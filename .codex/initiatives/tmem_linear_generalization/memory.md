...
- Latest direct scaled-MMAv5 root-format checkpoint, 2026-04-13 after the plain root `N=256` expansion: `test_tmem_runtime_matrix_mma_scaled_root_format_matrix` now covers every current scaled format pair (`mxfp8/mxfp8`, `mxfp4/mxfp4`, `mxfp8/mxfp4`, `mxfp4/mxfp8`, and `nvfp4/nvfp4`) at `N=128` and `N=256`, over both legacy and canonical TMEM-linear accumulator layouts. A scratch probe confirmed all twenty direct-root combinations match the dequantized reference and use the expected scaled-MMAv5 opcode counts (`4` ops for fp8/mixed vector-1X cases and `2` ops for homogeneous fp4/nvfp4 cases). Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; `make -j8` no-op success; exact new nodeid passed `20` selected cases across four GPU split groups (`5` each); tight `-k 'test_tmem_runtime_matrix_mma'` passed `243` selected cases across four GPU groups (`61`, `61`, `61`, `60`) in `52s`, `72s`, `106s`, and `83s`.
...
- Latest broad direct MMA/scaled-MMA runtime-matrix validation checkpoint, 2026-04-13 after the scaled root-format expansion: `python/test/gluon/test_tmem_runtime_matrix.py -k 'test_tmem_runtime_matrix_mma'` collected the pure MMA/scaled-MMA family (`243` cases) and passed across four GPU split groups (`61`, `61`, `61`, `60`). Do not use the older `-k 'mma and not cp'` phrase for pure MMA validation: collect-only showed it also selects `ldst_*mmav5*` cases because `mmav5` contains `mma`, which explains older broad-selector imbalance/timeouts.
