...
- Latest broad `tcgen05.cp` runtime-matrix validation checkpoint, 2026-04-13 after the scaled-copy `block_n=256` expansion: `python/test/gluon/test_tmem_runtime_matrix.py -k 'cp'` passed across four GPU split groups with aggregate `187 passed, 5 skipped` (`43 passed, 5 skipped`; `48 passed`; `48 passed`; `48 passed`). This supersedes older `167 passed, 5 skipped` broad-copy evidence; the increase is expected from adding all-format `block_n=256` scaled-copy cases.
- Latest scaled-MMAv5 copy-helper format/shape checkpoint, 2026-04-13 after the TMA TF32 negative expansion: `test_tmem_runtime_matrix_cp_scales_warpx4_via_scaled_mma_copy_matrix` now parameterizes `block_n` over `128` and `256` for all `CP_SCALES_WARPX4_FORMAT_PAIRS`, `num_ctas in {1,2}`, and legacy/canonical accumulator layouts. The exact copy-op count is now `(1 + block_n // 128) * (32 // vec_size)`, matching the extra B-scale N tile at `block_n=256` while preserving the existing count at `128`. Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `make` no-op success; exact expanded matrix across four GPU split groups (`40 passed` aggregate); nearby `-k 'cp_scales_warpx4_via_scaled_mma'` selector passed all `72` selected cases.
