...
- Current scales `ld/st` descriptor-view checkpoint, 2026-04-14 15:09 UTC: default/no-CGA tensor-memory scales allocations now support descriptor-view `ld/st` through the TMEM reshape algebra. `inferReshapeOpEncoding` routes scales through `inferTMemReshapeOpEncoding`, and view analysis treats scales roots as one-CTA only at the canonical-linear view materialization point. Positive runtime coverage covers `TensorMemoryScalesLayout()` roots at `(128,32)`, `(128,64)`, and `(256,64)` reshaped/permuted/reshaped into `tensor_memory_linear` views with exact root scales and view-linear opcode streams. Runtime-matrix collection is now `8227` tests: `cp=645`, `mma=2075`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2982`; bucketed evidence aggregates to `7776 passed, 451 skipped`. Do not infer support for scales multibuffering, non-default-CGA/two-CTA scales descriptor views, true scales `warpx2`, or low-shape row-anchor cases from this slice.
