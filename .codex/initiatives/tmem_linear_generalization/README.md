...
- Current scales `ld/st` descriptor-view checkpoint, 2026-04-14 15:09 UTC: default/no-CGA `TensorMemoryScalesLayout()` allocations now reshape through the TMEM descriptor-view algebra instead of falling into generic reshape inference. A scales root can be reshaped/transposed/reshaped into a canonical `tensor_memory_linear` view for `ld/st`, with runtime coverage at `(M,N) in {(128,32),(128,64),(256,64)}` and exact root scales plus linear-view load/store opcode assertions. Runtime-matrix collection is now `8227` tests: `cp=645`, `mma=2075`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2982`; bucketed evidence aggregates to `7776 passed, 451 skipped`. Validation: `make -j8`; py-compile; affected lit (`ops.mlir`, `invalid.mlir`, and `tritongpu_to_llvm_blackwell.mlir`) passed; focused collect `3/8227`; `ldst_scales` collect `98/8227`; focused split-4 selector passed the three non-empty groups; broader `ldst_scales` split-4 selector passed all `98`; `git diff --check` passed. Boundaries unchanged: scales multibuffering, non-default-CGA/two-CTA scales descriptor views, true scales `warpx2`, no-scales two-CTA `warpx2::02_13`, and low-shape row-anchor cases remain outside the positive set.
