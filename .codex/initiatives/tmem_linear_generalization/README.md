...
- Latest broad `tcgen05.cp` runtime-matrix validation checkpoint, 2026-04-13 after the scaled-copy `block_k=256` expansion: current head is green for `python/test/gluon/test_tmem_runtime_matrix.py -k 'cp'` across four GPU `pytest-split` groups. Aggregate result: `227 passed, 5 skipped` (`53 passed, 5 skipped`; `58 passed`; `58 passed`; `58 passed`). This supersedes older `187 passed, 5 skipped` broad-copy counts; the delta is expected from adding the all-format `block_k=256` scaled-copy matrix.
