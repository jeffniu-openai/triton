...
## Latest: 2026-04-24 runtime correctness closure

- Closed the staged-validation runtime correctness blocker around TMEM descriptor-view load/reduction chains. The core issue was that `OptimizeTMemLayouts` still treated full `memdesc_reshape`/`memdesc_trans` chains as legacy replayable producer views even when the current `MemDescType`/`tensor_memory_linear` layout was self-contained enough for direct type-local lowering. Same-view `store`/`load_min` and subword two-CTA x1 roundtrips could therefore apply a stale tensor transform after the TMEM load and return a permuted tile.
- The optimizer now keeps full-view replay as a legacy rescue only when direct type-local lowering has explicitly rejected the descriptor view. Supported linear/type-local views lower from the current SSA memdesc type/layout; the known M64 two-CTA scales boundary still uses the legacy replay rescue because the direct scale row-anchor rematerialization path is not implemented.
- Load+reduce fusion now follows legal tensor result chains through `convert_layout`, `reshape`, `trans`, and `abs`, deriving any reduced-result view transform only when the trailing reduction dimension is preserved. This keeps hardware `ld.red` available for the fixed view-chain case without using producer-chain replay as the semantic lowering path.
- The Gluon no-explicit-layout `load_min/load_max` software fallback now asks the descriptor for its ordinary register layout when hardware reduction is not legal, instead of reusing a reduction-oriented layout. The split-long-M reduction layout helper also refuses rank-2 `N < 4` descriptors so too-narrow software fallback cases do not reuse a hardware-reduction layout.
- Runtime tests were refreshed where they encoded old legacy replay packet choices rather than direct type-local lowering: scale descriptor-view ptx expectations now check the locally selected packets, and structural generic-pass expectations now assert the logical reshape/permute view result instead of replay-as-identity behavior.
- Validation: `make -j8`; `git diff --check`; lit `TritonNvidiaGPU/tmem_layouts.mlir` `1 passed`; structural fuzzer split-4 `36 passed`; full runtime matrix split-4 `1609 passed, 102 skipped`; `test_core.py -k tmem` split-4 `288 passed, 5 skipped`; example smokes `01-attention-forward.py` `2 passed` and `05-moe-bmm1-fused-gather.py` `2 passed`.

Previous final API cleanup checkpoint: as of 2026-04-23 22:56 UTC, the public `TensorMemoryUtils` API was trimmed so internal-only forwarding, scales-root, and type-local MMAv5 helper entry points became file-local implementation details. That checkpoint exposed the structural-fuzzer `ld.red` descriptor-view-chain blocker recorded below; the 2026-04-24 runtime correctness closure above fixes that blocker and supersedes the red staged-validation status.
...
Unsupported-direct ld/st diagnostics now use the active type-local raw-query
path and skip backing-row fallback for active self-contained descriptors.
Physical-bitcast type inference also uses the public raw-query helper, with
runtime coverage for selected active-subview bitcasts and updated TTGIR marker
...
cover both active self-contained subviews and scales descriptor views.
Validation: required `make -j8`; focused `ldst_scales and
(descriptor_view or reports)` selector `20 passed, 1687
deselected`; active-subview ld/st selector `12 passed, 1695
deselected`; four-GPU `ldst_scales and not reports` selector
passed as group1 `9 passed`, group2 `9 passed`, group3 `9 passed`,
group4 `6 passed`; lit `tmem_layouts.mlir` `1 passed`;
`git diff --check` passed.

Previous validation checkpoint: 2026-04-23 20:07 UTC tightened scales ld/st row-plan
...
`MemDescType` semantic path for active self-contained subviews and
scale descriptor views. Validation: required `make -j8`; focused
...
Previous validation checkpoint: 2026-04-23 20:01 UTC completed the scales ld/st
...
Previous validation checkpoint: 2026-04-23 05:44 UTC ran the broad non-scale
runtime selector `(ldst or ld_red or cp_no_scales) and not reports and not
scales` as a four-GPU split: group1 `124 passed, 26 skipped`, group2
`98 passed, 52 skipped`, group3 `130 passed, 20 skipped`, group4 `149 passed`.

Previous validation checkpoint: 2026-04-23 05:36 UTC made standalone
view/reg-layout query type inference active-subview local. Validation:
required `make -j8`; selected active-subview ld/st and ld.red rows `6 passed`;
selected/broad physical-bitcast selector `17 passed`; targeted lit set `6/6`.

Previous validation checkpoint: 2026-04-23 05:33 UTC made physical-bitcast type
inference active-subview local and added selected-subview coverage. Validation:
required `make -j8`; exact selected/preserved physical-bitcast rows `4 passed`;
broad `tmem_linear_runtime_views or physical_bitcast` selector `15 passed`;
targeted lit set `6/6`.

Previous validation checkpoint: 2026-04-23 05:28 UTC ran a broader active/scales
...
Previous validation checkpoint: 2026-04-23 05:23 UTC tightened ld.red
...
Previous validation checkpoint: 2026-04-23 04:55 UTC separated active-subview
...
`61 passed`, group2 `61 passed`, group3 `61 passed`, group4 `59 passed`.

Previous validation checkpoint: 2026-04-23 04:27 UTC added dynamic selected
...
Previous validation checkpoint: 2026-04-23 04:12 UTC added dynamic selected 2CTA
...
`warpx2::01_23` and `warpx2::02_13` schedules. Validation: required
`make -j8`; exact new rows `4 passed`; adjacent `warpx2` subview/indexed
selector `40 passed, 1598 deselected`; 4-GPU `cp_no_scales and not reports`
split passed as group1 `56 passed, 4 skipped`, group2 `60 passed`, group3
`60 passed`, group4 `57 passed`.

Previous validation checkpoint: 2026-04-23 04:00 UTC added dynamic selected copy
column-subview coverage. The new row selects between two same-typed
same-parent `128x128xf32` column subviews, issues `tcgen05.copy` into the
selected descriptor, and loads through that selected descriptor. Validation:
required `make -j8`; exact new row `2 passed`; adjacent copy subview/indexed
selector `23 passed, 1611 deselected`; 4-GPU `cp_no_scales and not reports`
split passed as group1 `55 passed, 4 skipped`, group2 `59 passed`, group3
`59 passed`, group4 `56 passed`.

Previous validation checkpoint: 2026-04-23 03:54 UTC added dynamic selected
...
existing exact-view fallback so high physical column bases are not collapsed to
zero. The active-subview planning helper only rewrites `allocShape` to the
logical shape for compact self-contained layouts, preserving wider support
images for exact layouts. MMAv5 active subview address/offset selection uses
the current type for self-contained descriptors. Validation: required
`make -j8`; exact scaled tile-permuted crash repro `1 passed`; full scaled
tile-permuted accumulator subslice function `10 passed`; exact two-CTA scaled
subslice row `1 passed`; 4-GPU positive MMAv5 selector passed as group1
...
Previous validation checkpoint: 2026-04-23 00:58 UTC completed the first
...
already-adjusted base-offset subtraction for active self-contained descriptors.
`copySharedToTmem` skips `getTMemSubviewRelativeBaseOffset` when copy planning
selected a type-local destination query. Validation: required `make -j8`;
exact `warpx2_01_23_twocta_subslice_view_positive` `4 passed`; focused ld/st
selector `78 passed`; focused ld.red selector `239 passed`; focused copy
selector `57 passed`; 4-GPU combined ld/st+ld.red+copy selector passed as
...
Previous validation checkpoint: 2026-04-23 00:25 UTC tightened active-subview
row-plan fallback behavior. `getTMemLdStRowPlanForRawQuery`,
`getTMemLdStDirectSupportTensorType`, and
`getTMemLoadReductionLayoutForMemDesc` no longer fall back to
`getBackingTMemLdStRowPlan` for active self-contained descriptors when their
type-local row plan is absent. Validation: required `make -j8`; exact
`warpx2_01_23_twocta_subslice_view_positive` `4 passed`; focused ld/st selector
`78 passed, 1547 deselected`; focused `ld_red and not reports and not scales`
selector `239 passed, 1386 deselected`; 4-GPU
`(ldst or ld_red) and not reports and not scales` split passed as group1
`120 passed, 28 skipped`, group2 `98 passed, 50 skipped`, group3
`128 passed, 20 skipped`, group4 `146 passed`; targeted lit set passed `6/6`.

Previous validation checkpoint: 2026-04-23 00:10 UTC completed the active-subview
ld/st support-query slice. `getTypeLocalTMemLdStSupportQueryPlan(MemDescType)`
returns the type-local query layout and row plan for active self-contained
descriptors, and `getTMemLdStSupportQueryPlan(Value)` now dispatches to it
before legacy support-image reconstruction. Validation: required `make -j8`;
exact `warpx2_01_23_twocta_subslice_view_positive` `4 passed`; focused ld/st
...
Previous validation checkpoint: 2026-04-22 23:53 UTC completed the active-subview
...
surrogate selection instead of walking to a parent backing plan. Legacy views
remain on the existing backing-plan path. Validation: required `make -j8`;
exact `warpx2_01_23_twocta_subslice_view_positive` `4 passed`; focused ld/st
...
Previous validation checkpoint: 2026-04-22 23:18 UTC completed the first
...
was a no-op. Checked-in pytest clean/error selector over invalid TMEM layouts,
copy, `ld.red`, plain MMAv5, and scaled-MMAv5 collected `200/1615` and passed
split-4 as `200 passed` (`50`, `50`, `50`, `50`). Non-Round-57 lit sweep
discovered `6` tests and ran as `5 passed, 1 failed`; the failure is existing
`FZ-20260421-0016` in `Conversion/relayout_tritongpu.mlir`. No false
unsupported diagnostic, assertion text drift outside the known bucket,
PassManager failure, vague/incorrect diagnostic drift, runtime miscompile, or
new independent `FZ-*`.
...
Previous fuzzing checkpoint: 2026-04-21 Round 58 high-CGA ownership lane B
completed. Report: `agents/fuzz_round58_high_cga_ownership_lane.md`. Required
`make -j8` was a no-op. Checked-in high-CGA selector collected `93/19729` and
ran as `87 passed, 6 skipped`; `membar-cluster.mlir` passed; temporary
ownership probe classified all `12` local-TMEM-in-larger-CGA rows as existing
`FZ-20260421-0010`. No missed `getModuleTwoCTAs` propagation bug, proxy/TMA/
mbarrier ownership regression, or new independent `FZ-*`.

Previous fuzzing checkpoint: 2026-04-21 16:01 UTC Round 58 plain MMAv5
accumulator-view lane C completed. Report:
`agents/fuzz_round58_plain_mma_acc_views_lane.md`. Required `make -j8` was a
no-op. Focused runtime-matrix selector over plain MMAv5 indexed accumulator
views, accumulator subslices, tile-permuted accumulator layouts, `use_acc`, and
2CTA variants collected `142/1615` and passed split-4 as `142 passed` (`36`,
`36`, `36`, `34`). Exact `test_core.py` controls collected `5/18114` and
passed. Existing `FZ-20260421-0003` and `FZ-20260421-0011` were not reproduced;
no compiler crash, verifier drift, false unsupported diagnostic, opcode/commit
mismatch, runtime miscompile, hang, unexpected skip/fail transition, or new
independent `FZ-*`.
...
completed split-4 as `3 passed, 23 xfailed`. No XPASS, unexpected failure, or
new independent `FZ-*`.

Previous fuzzing checkpoint: 2026-04-21 Round 39 TMA/multicast guardrail
completed. Report: `agents/fuzz_tma_multicast_guardrail_round39.md`.
Selector `tma_multicast or multicast_commit` over `python/test/gluon/test_core.py`
collected `11/18114` and passed as `11 passed`. No new independent `FZ-*`.

Previous fuzzing checkpoint: 2026-04-21 Round 39 allocation-lifetime guardrail
...
collected `255/1615` and passed split-4 as `255 passed`. No new independent
`FZ-*`.

Previous fuzzing checkpoint: 2026-04-21 Round 36 indexed-accumulator and
...
unexpected xfail/pass transition, or new independent `FZ-*` bucket was found.

Previous fuzzing checkpoint: 2026-04-21 Round 35 two-CTA proxy/commit and
...
and stayed stable as `9 passed, 24 xfailed`. Round 33 scale-bearing runtime
...
actual `ld.red` `32 pass, 12 clean unsupported, 4 parse failures`. Existing
`FZ-20260421-0019` now also covers rank-4 unit-prefix dimension aborts; new
candidate buckets are `FZ-20260421-0020` half-row optimizer signal,
`FZ-20260421-0021` half-column dimension abort, and `FZ-20260421-0022`
row-reversed half-row `ld.red` parse failure. No runtime wrong-result
miscompile was confirmed.
...
but narrowed `FZ-20260421-0014`: sequential wait-before-next-mbarrier-init
...
three regions. The plain-mbarrier follow-ups show the core bug is not
TMEM-copy-specific: one 2CTA copy mbarrier plus one independent plain mbarrier
reproduces in either order, and even two sequential plain-only mbarrier
intervals with no TMEM operations reproduce. Lane AS found no new bucket but
...
Additional Round 20 green baseline: `agents/fuzz_local_mma_twocta_round20.md`.
...
context diagnostics, and `2` `FZ-20260421-0014` proxy-fence crashes. The
`FZ-0014` rows show that two independent legal 2CTA no-scales copy/mbarrier
regions are enough to trigger the proxy-fence insertion abort, both for direct
and descriptor-chain destinations; scales copy is not required. No runtime
miscompile and no new independent `FZ-*` bucket was found.
...
- use `isa_api_followups.md` for TMEM-related ISA/API context that does not
  depend on linear-layout generalization, including items explicitly closed as
...
- 2026-04-21: completed Round 7 Lane A scaled-MMAv5 accumulator control-flow
  ...
  `/tmp/tmem_scaled_mma_controlflow_round7_probe.py`. The lane broadened
  `FZ-20260421-0007` across dynamic `if`, loop, helper-returned subslice,
  and indexed accumulator-view contrasts for `N in {32,64,128}`,
  ...
  exceptions overlap `FZ-20260421-0001`, so no new independent `FZ-*` id was
  assigned. No backend repair was attempted.
...
- 2026-04-20 20:21 UTC: tutorial polish added inline benchmark output and a
  ...
  tile-permuted `TensorMemoryLinearLayout` accumulator path used by the skinny
  projection benchmark was not expressible through the old compact
  `TensorMemoryLayout([BLOCK_M, BLOCK_N])` flow. Merge-base check:
  `11ee1144a737006921231bbd3386c187812c38e1` did not have tutorial `15` or
  `python/test/gluon/test_tmem_runtime_matrix.py`; the preexisting scaled-MMA
  tutorial used `TensorMemoryLayout` for accumulators and did not expose
  `TensorMemoryLinearLayout` tile-permuted narrow fragments. Validation:
  py-compile, `git diff --check`, focused tutorial pytest `6 passed in
  6.33s`, and script benchmark output `N=32` `1.18x`, `N=64` `1.05x`.
- 2026-04-20 20:10 UTC: reframed
  ...
  `0.013 ms` (`1.05x`). Tutorial rewrite checkpoint commit `021688bc2`,
...
- 2026-04-18 07:16 UTC: closed the historical i8 MMAv5 Gap #1 for this
  ...
  (independent A/B signedness and integer saturation controls) is explicitly
  out of scope for the TMEM linear-layout generalization project. The current
  active linear-layout Gap #1 remains the `tcgen05.cp` complete-support
  umbrella after the 07:13 UTC renumbering; this checkpoint closes only the
  former i8 item.
...
- 2026-04-18 01:51 UTC: added the first checked-in runtime pytest coverage for
  the historical Gap #1 signed i8 MMAv5 item. `test_core.py` now has an exact
  `GB200/sm100` gated positive test over three small shapes (`64x128x32`,
  `128x128x32`, `128x256x64`) with exact int32 reference comparison and
  `kind::i8` PTX/LLIR checks. The old i8 clean-error tests are now restricted
  to Blackwell Ultra/sm103 so they do not contradict GB200 support.
- 2026-04-18 01:28 UTC: the historical Gap #1 signed i8 MMAv5 item is
  runtime-validated on GB200 through `caas-gpu10` with image `cudaberry-arm`.
  `nvidia-smi` first confirmed four `NVIDIA GB200` devices with compute
  capability `10.0`; the actual execution probe used one GB200, uploaded
  ...
  inspection.
- 2026-04-18 00:01 UTC: prepared the historical Gap #1 GB200 validation
  artifact under `experiments/mmav5_i8_remote/`. Local `sm_100` compilation
  generates `tcgen05_i8_signed_sm100.ptx` with `.target sm_100a`,
