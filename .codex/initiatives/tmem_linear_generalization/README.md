...
Validation for this batch and follow-up: `make -j8`; lit `test/TritonNvidiaGPU/invalid.mlir`, `test/TritonNvidiaGPU/canonicalize.mlir`, `test/TritonNvidiaGPU/ops.mlir`, `test/TritonGPU/nvidia-fpsan.mlir`, `test/Analysis/test-buffer-region.mlir`, and `test/Conversion/tritongpu_to_llvm_tmem_control_flow.mlir` passed in focused runs; Gluon frontend bitcast tests passed `3 passed`; focused representative physical-bitcast/runtime rows passed `4 passed`; the 4-GPU GB200-style Python sample selected `1927` cases and passed across all shards (`382 passed, 100 skipped`; `458 passed, 24 skipped`; `482 passed`; `481 passed`). `git diff --check` passed. Known validation boundary: `test/Conversion/tritongpu_to_llvm_blackwell.mlir` still fails before the updated reinterpret section on the existing `store_packedb16_4x32xf16` sub-32-bit TMEM view-origin legalization gap, then cascades into stale FileCheck misses.
...
- The previous M64 two-CTA tensor-memory-scales `32x32b` descriptor-view rescue is now a clean unsupported compile-time diagnostic until direct row-anchor rematerialization or packet-footprint modeling is implemented from current type/layout facts. Structural fuzzer rows that relied on full-view replay are now clean-negative diagnostics instead of execution positives.
- Validation used normal per-GPU `TRITON_CACHE_DIR` cache behavior, not `TRITON_ALWAYS_COMPILE`: `make -j8`; focused scales descriptor-view selector `13 passed`; structural fuzzer split-4 `36 passed`; lit `TritonNvidiaGPU/tmem_layouts.mlir` `1 passed`; full runtime matrix split-4 `346 passed, 82 skipped` / `408 passed, 20 skipped` / `428 passed` / `427 passed`; `test_core.py -k tmem` split-4 `69 passed, 5 skipped` / `74 passed` / `74 passed` / `71 passed`; `git diff --check`.
