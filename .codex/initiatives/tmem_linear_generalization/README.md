...
- Latest direct scaled-MMAv5 root-format checkpoint, 2026-04-13 after the plain root `N=256` expansion: one-CTA root accumulator coverage now spans every current scaled format pair (`mxfp8/mxfp8`, `mxfp4/mxfp4`, `mxfp8/mxfp4`, `mxfp4/mxfp8`, and `nvfp4/nvfp4`) at `N=128` and `N=256`, over both legacy and canonical TMEM-linear accumulator layouts. A scratch probe confirmed all twenty combinations match the dequantized reference and keep the same scaled-MMA opcode counts as the existing direct root path (`4` ops for fp8/mixed vector-1X cases and `2` ops for homogeneous fp4/nvfp4 cases). The runtime matrix now pins this through `SCALED_MMA_ROOT_FORMAT_CASES`. Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; `make -j8` no-op success; exact new nodeid passed all `20` parameters across four GPU split groups (`5` each); tight `-k 'test_tmem_runtime_matrix_mma'` passed `243` selected cases across four GPU groups (`61`, `61`, `61`, `60`) in `52s`, `72s`, `106s`, and `83s`.
...
- Latest broad direct MMA/scaled-MMA runtime-matrix validation checkpoint, 2026-04-13 after the scaled root-format expansion: use `-k 'test_tmem_runtime_matrix_mma'` for the pure MMA/scaled-MMA family, not the historical `-k 'mma and not cp'` selector. Collect-only showed `-k 'mma and not cp'` also selects `ldst_*mmav5*` cases because `mmav5` contains `mma`, which explains earlier static-split imbalance. The tight selector now collects `243` cases and passed across four GPU split groups: `61`, `61`, `61`, and `60` passed.

- Latest broad `tcgen05.cp` runtime-matrix validation checkpoint, 2026-04-13 after the scaled-copy `block_n=256` expansion: current head is green for `python/test/gluon/test_tmem_runtime_matrix.py -k 'cp'` across four GPU `pytest-split` groups. Aggregate result: `187 passed, 5 skipped` (`43 passed, 5 skipped`; `48 passed`; `48 passed`; `48 passed`). This supersedes older `167 passed, 5 skipped` broad-copy counts; the delta is expected from adding the all-format `block_n=256` scaled-copy matrix.

- Latest scaled-MMAv5 copy-helper format/shape checkpoint, 2026-04-13 after the TMA TF32 negative expansion: `test_tmem_runtime_matrix_cp_scales_warpx4_via_scaled_mma_copy_matrix` now covers `block_n=128` and `256` for every existing scaled format pair, both `num_ctas=1/2`, and both legacy/canonical accumulator layouts. A probe showed the `block_n=256` two-CTA `mxfp8/mxfp8` case emits three `tcgen05.cp...warpx4.32x128b` copies instead of two because the B-scale copy needs a second N tile while the A-scale copy count is unchanged; the test now pins `expected_count = (1 + block_n // 128) * (32 // vec_size)`. Validation: `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `make` no-op success; exact expanded matrix across four GPU split groups (`40 passed` aggregate); nearby `-k 'cp_scales_warpx4_via_scaled_mma'` selector passed all `72` selected copy-matrix plus geometry cases across four groups.
