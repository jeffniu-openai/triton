...
- Latest broad direct MMA/scaled-MMA runtime-matrix validation checkpoint, 2026-04-13 after the broad copy validation: use `-k 'test_tmem_runtime_matrix_mma'` for the pure MMA/scaled-MMA family, not the historical `-k 'mma and not cp'` selector. Collect-only showed `-k 'mma and not cp'` also selects `ldst_*mmav5*` cases because `mmav5` contains `mma`, which explains earlier static-split imbalance. The tight selector collected `201` cases and passed across four GPU split groups: `51`, `51`, `51`, and `48` passed.
