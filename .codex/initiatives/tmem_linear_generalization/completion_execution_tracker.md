...
- [x] Shared type-local ld/st predicate slice: added
  `hasTypeLocalTMemLdStLayout(MemDescType)` and routed query, row-plan,
...
Current MMAv5/scales runtime checkpoint:
...
## 2026-04-25 05:52 UTC: broad TMEM recovery validation green

Root causes found and fixed after the first broad rerun:
- `test_core.py::test_block_m_64_mma[legacy]` exposed an over-broad MMAv5 verifier check. The check used allocated physical rows to reject blockM=64 interleaved-M64 layouts, but the legal main-green pattern slices the current LHS operand to a logical 64-row tile and manually duplicates A across two TMEM allocations. The verifier now rejects only when the current logical A tile is not 64 rows while the instruction plan is blockM=64 and the accumulator is interleaved.
- Split-N f16 and tensor-memory-scales explicit variants exposed a frontend design bug: the early dword-footprint diagnostic counted logical exposed dwords and rejected packed sub-32-bit layouts before the type-local planner could prove a packed realization. That precheck now applies only to 32-bit-or-wider element descriptors; packed f16/i8 cases are decided by the planner.
- Higher-rank half-row descriptor views exposed an alloc-shape preservation weakness. Indexing a narrowed leading dimension outside the encoding rank can hide a runtime row-origin bit in the current `taddr`. The result type must preserve that as row allocation extent when the remaining row span is smaller than a full direct ld/st row footprint, so direct ld/st rejects the non-self-contained row slice instead of silently miscompiling. Column-split/index views whose remaining row span already covers the full footprint still fold the hidden extent into columns and remain positive.
- Several runtime-matrix rows had stale IR-shape expectations: two-CTA multidim slice positives no longer require `tt.trans`/`tt.split`/`tt.join` to survive, and the old replay-only `32x32` multidim slice rows are now clean unsupported because parent replay is disallowed and direct type-local ld/st cannot materialize that compact view.

Validation completed:
- `make -j8` rebuilt `TensorMemoryUtils.cpp`, `Dialect.cpp`, and `gluon_ir.cc` successfully.
- `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py` passed.
- `python/test/gluon/test_core.py::test_block_m_64_mma` -> `2 passed`.
- `lit -v test/TritonNvidiaGPU/invalid.mlir` -> `1 passed`, preserving the true invalid interleaved blockM=64 rejection.
- Focused runtime-matrix recovery selector -> `44 passed, 1664 deselected`.
- Full broad split over `python/test/gluon/test_core.py`, `python/test/gluon/test_tmem_runtime_matrix.py`, and `python/test/gluon/test_tmem_structural_fuzzer.py` is green on the current build: group1 `4347 passed, 620 skipped, 14898 deselected`; group2 `2671 passed, 2296 skipped, 14898 deselected`; group3 `2359 passed, 2608 skipped, 14898 deselected`; group4 `3575 passed, 1389 skipped, 14901 deselected`.
- `git diff --check` passed.
