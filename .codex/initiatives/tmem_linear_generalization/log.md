...
- The new clean-negative matrix covers direct scaled-MMAv5 accumulator layouts `128x32/tile_n=8` and `128x64/tile_n=16` across all current scaled format pairs and `K in {128,256}`.
- Probe result recorded in code/tests: these narrow direct tile-permuted layouts fail the directly-supported block-scaled tensor-memory-layout diagnostic, while the existing `128x128/tile_n=32` matrix remains the separate repeated-`N=32` scale-fragment boundary.
- Validation:
  - `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py` -> passed;
  - `git diff --check` -> passed after docs;
  - `make -j8` -> no work to do;
  - no-PYTHONPATH focused collect selected `20/5987` for `scaled_acc_tile_permuted_narrow`;
  - no-PYTHONPATH tight MMA collect selected `1358/5987`;
  - no-PYTHONPATH full-file collect reported `5987`;
  - focused `scaled_acc_tile_permuted_narrow` selector passed all `20` cases across split-4 on four GPUs (`5` per group; `5.82s`, `5.90s`, `5.69s`, `6.08s`).
...
- Expanded `test_tmem_runtime_matrix_mma_acc_tile_permuted_n64_reports_clean_unsupported` into the narrow tile-permuted clean-negative matrix for plain MMAv5.
- The new matrix covers `128x32/tile_n=8` beside the existing `128x64/tile_n=16`, across every supported plain operand kind, `K in {32,64}`, and no-accumulator plus `use_acc=True`.
- Validation:
  - `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py` -> passed;
  - `git diff --check` -> passed after docs;
  - `make -j8` -> no work to do;
  - no-PYTHONPATH focused collect selected `40/5967` for `tile_permuted_narrow`;
  - no-PYTHONPATH tight MMA collect selected `1338/5967`;
  - no-PYTHONPATH full-file collect reported `5967`;
  - focused `tile_permuted_narrow` selector passed all `40` cases across split-4 on four GPUs (`10` per group; `5.76s`, `5.98s`, `5.90s`, `5.76s`).
...
- Committed and pushed higher-rank `ld/st` OOR guard auto coverage:
  - `e2d793726`
  - branch / remote:
    - `codex/tmem`
    - `origin/codex/tmem`
- Scope:
  - add `auto` instruction-selection coverage to higher-rank resource-boundary
    tests after the positive higher-rank auto matrices were broadened.
- Implementation:
...
- Committed and pushed same-row/column permuted `ld/st` auto coverage:
  - `c83d358e4`
  - branch / remote:
    - `codex/tmem`
    - `origin/codex/tmem`
- Scope:
  - add `auto` instruction-selection coverage to `LDST_PERMUTED_CASES`.
- Implementation:
  - `LDST_PERMUTED_CASES` now uses `LDST_VARIANTS`;
  - this reaches both the direct ld/st permuted-layout sweep and the
    descriptor-chain composition permuted-layout sweep.
- Validation:
  - build:
    - `CPLUS_INCLUDE_PATH=/usr/include/c++/13:/usr/include/aarch64-linux-gnu/c++/13 make -j8`
    - `PASSED`, ninja reported no work to do
...
- Committed and pushed higher-rank dim0-slice `ld/st` auto coverage:
  ...
  - branch / remote:
    ...
    - `origin/codex/tmem`
- Scope:
  - add `auto` instruction-selection coverage to the remaining simple
    higher-rank dim0-slice positive matrices.
- Implementation:
...
- Added a pure row-permuted `tcgen05.ld.red` N-sweep.
- Source/test change:
  - added `LD_RED_PURE_ROW_PERMUTED_N_SWEEP_CASES` for non-identity row
    permutations at `N=64` and `N=256`;
  - added `test_tmem_runtime_matrix_ld_red_pure_row_permuted_n_sweep`, keeping
    the existing row/column cross-product test at `128x128` unchanged.
- Probe result before promotion:
  - `N=64` pure row permutations emit one
    `tcgen05.ld.red.sync.aligned.32x32b.x64.min.f32` op;
  - `N=256` pure row permutations emit four
    `tcgen05.ld.red.sync.aligned.32x32b.x64.min.f32` ops;
  - no non-`32x32b` reduction atom was discovered.
- Validation:
  - build:
...
## 2026-04-13 13:20 UTC: corrected two-CTA `warpx2::02_13` direct-PTX probe

- Found the same probe-shape flaw that affected the scales `warpx2` experiment:
  - `compile_seed_kernel` launched the canonical two-CTA `warpx2::01_23` copy while compiling;
  - patched `02_13` cubins were therefore tested after a prior canonical kernel launch;
  - the harness now uses `warmup(...)` by default and has `--prime-canonical` only for historical-artifact reproduction.
- Regenerated durable artifacts:
  - `experiments/results/probe_cp_warpx2_02_13_twocta_direct_ptx_current.log`;
  - `experiments/results/probe_cp_warpx2_02_13_twocta_source_offsets_*_gpu*.jsonl`.
- Corrected results:
  - all `22` named opcode/descriptor/two-message variants execute with finite output, but none match the extended two-CTA `02_13` oracle; all duplicate one source-column pair;
  - the four-GPU source-offset sweep covers `sourceOffsetB128` `36..127` with destination deltas `0` and `4`, `184` total records;
  - `74` records for offsets `36..72` execute but all duplicate one source-column pair;
  - `110` records for offsets `73..127` launch-fail in isolated child processes;
  - there are zero oracle matches, zero sentinel records, and no NaN-bearing successful records in the corrected run.
- Spot priming check:
  - `opcode_only --prime-canonical` and `sourceOffsetB128=64,dst=0 --prime-canonical` still duplicate the same source-column pair; priming does not rescue these sampled two-CTA `02_13` variants.
- Interpretation:
  - the 2026-04-13 09:42 NaN/partial-duplicate breakdown is superseded; it was generated by a probe path that launched canonical `01_23` before patched cubins;
  - public two-CTA `warpx2::02_13` should remain clean unsupported;
  - the next useful work is descriptor/address-message synthesis from the layout model, not another small direct-seed offset toggle.
- Validation / hygiene:
  - `python3 -m py_compile .codex/initiatives/tmem_linear_generalization/experiments/probe_cp_warpx2_02_13_twocta_direct_ptx.py` passed;
  - representative paired unprimed/primed probes passed as described above;
...
- Added `LDST_TWOCTA_I32_BROAD_CASES` and `test_tmem_runtime_matrix_ldst_twocta_i32_broad_linear_layouts`.
- The new coverage spans `block_two_ctas` and MMAv5-like two-CTA `256x{64,128,256}` TMEM-linear layouts, direct and descriptor-chain modes, and `auto`, `32x32b`, `16x64b`, `16x128b`, and `16x256b` variants using int32 payloads.
- This complements the existing broad two-CTA f32 direct/descriptor tests instead of rewriting their nodeids.
- Validation:
  - `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`;
  - `make -j8` -> no work to do;
  - no-PYTHONPATH focused collect selected `60/3789`;
  - no-PYTHONPATH `ldst` collect selected `1902/3789`;
  - focused selector passed all `60` cases across four GPUs (`15` per group; group times about `74.63s`, `21.09s`, `304.62s`, and `152.89s`);
  - `git diff --check` passed.
...
- Expanded `LDST_ROWCOL_PERMUTED_CASES` so the existing f32 full row/column cross-product remains intact and i32 now covers every non-identity broad row/column layout.
- This covers direct and descriptor-chain `128x{64,128,256}` roundtrips over every public `ld/st` variant for pure row, pure column, and mixed non-identity row+column permutations. Identity/identity i32 remains covered by the broad identity/mixed i32 tests rather than duplicated here.
- No compiler/lowering source changed in this slice; it is a coverage expansion over already-supported lowering behavior.
- Validation:
  - `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`;
  - `git diff --check`;
  - `make -j8` -> no work to do;
  ...
  - focused mixed row/column i32 selector passed all `270` cases across split-16 on four GPUs (`17` cases for groups 1-15 and `15` cases for group 16; group times `95.87s`, `90.37s`, `98.35s`, `51.10s`, `91.40s`, `95.88s`, `98.71s`, `9.62s`, `425.76s`, `417.78s`, `423.54s`, `212.60s`, `422.09s`, `430.72s`, `435.20s`, and `4.17s`).
...
- Extended `tmem_mma_lhs_kernel` and `tmem_mma_lhs_subslice_kernel` with `ACC_INIT` and `USE_ACC` constexprs. Existing no-accumulator callers pass `0.0/False`; new use-acc callers initialize the accumulator to `1.0` and call `tcgen05_mma(..., use_acc=True)`.
- Added `test_tmem_runtime_matrix_mma_lhs_tile_permuted_use_acc`, covering the existing full-shape tile-permuted TMEM-LHS matrix over all supported plain operand kinds, `N in {64,128,256}`, and `K in {128,256}` except the existing TF32 shared-memory OOR row.
- Added `test_tmem_runtime_matrix_mma_lhs_subslice_view_plain_kinds_use_acc`, covering every supported plain operand kind, legacy/canonical accumulator layouts, `N in {64,128,256}`, and `K in {32,64}` through the supported `ttg.memdesc_subslice` TMEM-LHS path.
...
## 2026-04-14 11:25 UTC: ld.red descriptor-chain N-width all-layout explicit variants

- Expanded `LD_RED_DESCRIPTOR_CHAIN_N_SWEEP_EXPLICIT_VARIANT_CASES` from identity plus `rowcol_rotate_reverse` to all descriptor-chain N-sweep layouts: identity, tile-permuted, pure column reverse, pure row reverse, and mixed row/column reverse.
- This adds `144` cases and completes the `N in {64,256}` explicit-variant cross product over `32x32b`, `16x32bx2`, and `32x32b_splitn`, both `min`/`max`, and all legal modifier modes. The existing `N=128` descriptor-chain explicit matrix already covered these layouts.
- Runtime values and reductions passed for the added layouts. The only first-pass failures were opcode-offset assertion mismatches for `col_reverse`, `N=256`, `16x32bx2` / `32x32b_splitn`: like `rowcol_rotate_reverse`, those split variants legally emit offsets `[0, 128, 64, 192]`, so the expected-offset exception now covers both layouts.
...
- Extended `mma_scaled_tcgen05_copy_kernel` in `python/test/gluon/tmem_test_utils.py` with `ACC_INIT`; nonzero init stores a full accumulator seed and makes the first scaled MMA chunk accumulate from that seed while preserving the old zero-init behavior for existing callers.
- Added `test_tmem_runtime_matrix_cp_scales_warpx4_via_scaled_mma_copy_matrix_use_acc`, mirroring the existing scaled-copy helper matrix across all current scaled format pairs, `blockN/blockK in {128,256}`, `num_ctas in {1,2}`, multicast false/true, and legacy/canonical accumulator layouts.
...
- Validation: `make -j8`; `python3 -m py_compile python/test/gluon/tmem_test_utils.py python/test/gluon/test_tmem_runtime_matrix.py`; `git diff --check`; no-PYTHONPATH focused use-acc collect selected `160/7331`; no-PYTHONPATH `cp_scales_warpx4` collect selected `354/7331`; no-PYTHONPATH full-file collect reported `7331`; focused use-acc selector passed all `160` cases across split-4 (`40` per group; group times `51.74s`, `52.20s`, `52.83s`, and `52.43s`); adjacent `cp_scales_warpx4` selector passed all `354` cases across split-4 (`89`, `89`, `89`, and `87` selected; group times `73.89s`, `88.59s`, `89.01s`, and `46.18s`).
- Next: commit/push this scaled-copy use-acc checkpoint, then move to another exact non-parked ISA coverage slice.
## 2026-04-14 12:50 UTC: ld.red minimal-N descriptor/direct explicit variants

- Added `N=32` to the descriptor-chain `ld.red` N-width sweep for identity, tile-permuted, pure column reverse, pure row reverse, and mixed row/column reverse layouts.
- Added the same minimal-N rows to the shared explicit-variant table used by both descriptor-chain and direct `ld.red`, covering `32x32b`, `16x32bx2`, and `32x32b_splitn` across `min`/`max` and all legal modifier modes.
...
## 2026-04-14 12:26 UTC: M64 MMAv5 accumulator subviews from wider linear parents

- Fixed MMAv5 accumulator view planning for `blockM=64` linear layouts when a `ttg.memdesc_subslice` narrows N from a wider parent allocation. The verifier now restricts the candidate layout to the active view shape and drops column input bases that only select columns outside that view, while preserving zero bases already present in the allocation layout. This keeps the layout as the source of truth and avoids adding op attributes or root-marker hacks.
- Added `test/TritonNvidiaGPU/ops.mlir` coverage for a `64x256` accumulator subview cut from a `64x512` M64 linear parent and consumed by `ttng.tc_gen5_mma`.
- Added `MMA_M64_ACC_SUBSLICE_CASES` and `test_tmem_runtime_matrix_mma_m64_acc_subslice_view_plain_kinds` in `python/test/gluon/test_tmem_runtime_matrix.py`, covering every supported plain kind, `N in {64,128,256}`, `K in {32,64}`, both slice starts, and `use_acc in {False,True}`.
...
- Expanded executable rank-5 descriptor-chain `ld/st` coverage to the minimal `N=32` opcode family for defined layouts: single-CTA identity plus two-CTA block/MMAv5-like layouts. The existing single-CTA mixed family remains `N>=64` because its layout helper encodes column bases that require at least 64 columns.
...
## 2026-04-14 14:57 UTC: copy warpx2 two-CTA subview coverage

- Added two-CTA no-scales `warpx2` parent-layout helpers, `tmem_copy_no_scales_warpx2_twocta_subslice_view_kernel`, and paired runtime tests in `python/test/gluon/test_tmem_runtime_matrix.py`.
- Positive coverage: `tcgen05.cp.cta_group::2.warpx2::01_23.64x128b` copies from a canonical two-CTA `[256,8]` TMEM-linear parent sliced to an active `[256,4]` view at `slice_start in {0,4}`, for both `f32` and `i32`. The test asserts exact copy and multicast commit opcodes, no `cta_group::1`, no `02_13` opcode leakage, and generic `ttg.memdesc_subslice` rather than legacy `ttng.tmem_subslice`.
- Clean-negative coverage: the analogous two-CTA `warpx2::02_13` subview parent remains unsupported and now asserts the same descriptor/address diagnostic as the direct path, including the high source-column bit and the invalidity of decomposing a two-CTA TMEM view into `cta_group::1` copies.
- Scratch probe `/tmp/probe_warpx2_view_gaps.py` also checked indexed `warpx2` view shapes. Direct lifted `parent.index(1)` failed result-type inference with a rank/preserved-encoding error; `slice(..., dim=0).index(0)` reached descriptor views with an inactive zero column basis and failed before copy lowering. Treat that as a separate generic indexed-view inference/algebra gap, not as an immediate `warpx2` copy support path.
- Current runtime-matrix collection is `8224` tests: `cp=645`, `mma=2075`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2979`; current bucketed evidence aggregates to `7773 passed, 451 skipped`.
- Validation completed: `make -j8`; `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; no-PYTHONPATH focused collect selected `8/8224`; no-PYTHONPATH full `warpx2` collect selected `51/8224`; no-PYTHONPATH CP collect selected `645/8224`; no-PYTHONPATH full-file collect reported `8224`; focused selector passed all `8` cases across split-4 (`2` per group); full `warpx2` selector passed all `51` cases across split-4 (`13`, `13`, `13`, and `12`); full CP selector passed/skipped `635 passed, 10 skipped` across split-4 (`152 passed/10 skipped`, `162 passed`, `162 passed`, and `159 passed`); `git diff --check` passed.
- Next: run final hygiene, commit/push this bounded `warpx2` descriptor-view checkpoint, then continue another non-parked TMEM ISA coverage slice. Hard frontiers remain true tensor-memory-scales `warpx2` and no-scales two-CTA `warpx2::02_13` until a real descriptor/address/staging model appears.

## 2026-04-14 15:09 UTC: scales ld/st descriptor-view support

- Routed tensor-memory scales `memdesc_reshape` through the TMEM descriptor-view algebra instead of generic TritonGPU reshape inference, which previously failed before any scales descriptor-view `ld/st` path could compile.
- Kept scales-specific one-CTA handling local to `getTMemViewAnalysisLayout`: scales encodings carry CGA layout but no legacy `twoCTAs` bit, while the supported descriptor view materializes as canonical `tensor_memory_linear` for default/no-CGA scales roots.
- Added `test/TritonNvidiaGPU/ops.mlir` coverage for a `128x32xi8` scales memdesc reshaped to a `64x2x32xi8` TMEM-linear descriptor view.
- Added runtime matrix coverage for a scales root store, descriptor-view load/store update, and root load-back at `(M,N) in {(128,32),(128,64),(256,64)}` with exact root scales opcode families and exact linear-view `32x32b` load/store families.
- Current runtime-matrix collection is `8227` tests: `cp=645`, `mma=2075`, splitn/misc `=571`, `ld_red=1954`, and `ldst=2982`; current bucketed evidence aggregates to `7776 passed, 451 skipped`.
- Validation completed: `make -j8`; `python3 -m py_compile python/test/gluon/test_tmem_runtime_matrix.py`; `lit -v test/TritonNvidiaGPU/ops.mlir`; `lit -v test/TritonNvidiaGPU/invalid.mlir test/TritonNvidiaGPU/ops.mlir test/Conversion/tritongpu_to_llvm_blackwell.mlir`; no-PYTHONPATH exact collect selected `3/8227`; no-PYTHONPATH `ldst_scales` collect selected `98/8227`; exact split-4 selector passed the three non-empty groups; broad `ldst_scales` selector passed all `98` cases across split-4 (`25`, `25`, `25`, and `23`); `git diff --check` passed.
- Boundaries unchanged: scales multibuffering still reports clean unsupported, non-default-CGA/two-CTA scales descriptor views are not positive coverage, true scales `warpx2` remains parked, no-scales two-CTA `warpx2::02_13` remains parked, and low-shape row-anchor cases remain unsupported until there is a real layout/planner path.
- Next: commit/push this compiler/runtime coverage slice, then continue the next non-parked TMEM ISA coverage gap.


## 2026-04-14 15:32 UTC: scales CGA descriptor-view fallback boundary

- Followed the default/no-CGA scales descriptor-view support with non-default-CGA probes for explicit `32x32b` `ld/st` views.
- The first probe result was dangerous: `(128,64)` failed cleanly, but `(256,32)` and `(256,64)` compiled through a type-only standalone query fallback and produced wrong runtime data (`6144/8192` and `12288/16384` mismatches). The fallback had canonicalized the query type and erased the support/broadcast physical projection.
- Updated TMEM view/query analysis to derive scales `twoCTAs` from `TensorMemoryScalesLayout` CGA layout. Added verifier and Gluon frontend guards so two-CTA 8-bit descriptor views with nontrivial block dimensions and zero row/column support bases require an exact support/raw-query physical projection; if that cannot be proven, `get_reg_layout("32x32b")` reports clean unsupported instead of using type-only layout recovery.
- Added clean-negative runtime-matrix coverage for CGA scales descriptor views at `(128,64)`, `(256,32)`, and `(256,64)` with `cga_layout=((1,0),)`, `num_ctas=2`, and explicit `32x32b`.
...
## 2026-04-14 16:27 UTC: two-CTA plain MMAv5 indexed-accumulator K=128 coverage

- Expanded `MMA_TWOCTA_INDEXED_ACC_CASES` in `python/test/gluon/test_tmem_runtime_matrix.py` from `K in {32,64}` to `K in {32,64,128}` for both the live-depth parent rows and the `linear_unit_parent` `N=256` construction.
- The two-CTA accumulator `memdesc_index` matrix now covers every supported plain kind, legacy parent layouts at `N in {64,128,256}`, canonical linear parent layouts at `N in {64,128}`, canonical linear unit parents at `N=256`, and both no-accumulator plus `use_acc=True` paths at `K=128`.
- This is test-only descriptor-view saturation over already-supported lowering. It does not claim K128 coverage for two-CTA accumulator subviews, TMA-fed matrices, or scaled-MMAv5.
...
## 2026-04-14 16:39 UTC: plain MMAv5 TMEM-LHS subview K=128 coverage

- Expanded `MMA_LHS_SUBSLICE_NK_CASES` in `python/test/gluon/test_tmem_runtime_matrix.py` from `K in {32,64}` to `K in {32,64,128}`.
- The TMEM-LHS `memdesc_subslice` matrix now covers every supported plain kind, legacy/canonical accumulator layouts, `N in {64,128,256}`, and both zero-accumulator plus `use_acc=True` paths at `K=128` through `[128,2*K]` LHS parents.
- This is test-only descriptor-view saturation over already-supported lowering and closes the remaining obvious plain-MMAv5 K128 descriptor-view gap.
...
## 2026-04-14 17:14 UTC: scaled two-CTA MMAv5 block_n=64 accumulator subviews

- Fixed the two-CTA scaled-MMAv5 accumulator-subview helper for active N tiles narrower than the packed scale descriptor minimum. Scale TMEM allocation/copy now uses descriptor-derived padded extents (`REP_MN * 128`) instead of the active MMA N extent, and the Python wrapper builds scale descriptors with at least 128 rows.
- Expanded `SCALED_MMA_TWOCTA_ACC_SUBSLICE_K_CASES` from fixed `block_n=128` to `block_n in {64,128}` with `parent_n=2*block_n`, both slice starts, `block_k in {128,256}`, multicast false/true, and every current scaled format pair. Both zero-accumulator and `use_acc=True` tests consume the expanded table.
- The old `block_n=64`, `parent_n=128` scratch failure is no longer parked: the zero descriptor dimension came from using the active 64-row scale tile as the packed descriptor block size instead of using the descriptor-minimum 128-row padded scale tile.
