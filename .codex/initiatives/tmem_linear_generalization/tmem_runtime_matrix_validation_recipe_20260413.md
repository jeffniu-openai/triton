...
Full collection with `PYTHONPATH` unset reports `5552` tests after the 2026-04-14 staged CP, `ld.red`, `ld/st`, MMAv5, and scaled-MMAv5 coverage expansions. The coverage-preserving bucket split is:

| Bucket | Selector | Cases | Scheduling |
| --- | --- | ---: | --- |
| `cp` | `-k cp` | 381 | split 4, one process per GPU |
| `mma` | `-k test_tmem_runtime_matrix_mma` | 980 | split 4, one process per GPU |
...
The buckets sum to all `5552` collected tests. The `splitn` bucket must use exact nodeids; plain `-k splitn` also matches parameter IDs such as `32x32b_splitn` inside `ld_red` and `ld/st`, which pollutes the timing profile.
...
- `mma`: after the 2026-04-14 scaled-MMAv5 indexed-accumulator descriptor-view expansion, the tight MMA bucket collects `980` cases. The latest focused scaled indexed-accumulator selector passed `30` cases across four groups (`8`, `8`, `8`, and `6`) in `4.59s`, `6.07s`, `6.52s`, and `4.58s`; the prior plain indexed-accumulator selector passed `100` cases across four groups (`25` per group) in `4.32s`, `5.77s`, `6.60s`, and `7.57s`.
...
Aggregating the current per-bucket evidence gives full matrix coverage: `5106 passed, 446 skipped` across all `5552` collected cases. This is bucketed evidence from focused/bucket reruns, not a reduced matrix claim; refresh the full runner after shared lowering or major scheduling changes.
