...
Full collection with `PYTHONPATH` unset reports `3297` tests after the 2026-04-14 CP, `ld.red`, scaled-MMAv5, and copy `warpx2` dtype coverage expansions. The coverage-preserving bucket split is:

| Bucket | Selector | Cases | Scheduling |
| --- | --- | ---: | --- |
| `cp` | `-k cp` | 321 | split 4, one process per GPU |
...
The buckets sum to all `3297` collected tests. The `splitn` bucket must use exact nodeids; plain `-k splitn` also matches parameter IDs such as `32x32b_splitn` inside `ld_red` and `ld/st`, which pollutes the timing profile.
...
Aggregating the current per-bucket evidence gives full matrix coverage: `2851 passed, 446 skipped` across all `3297` collected cases. This is a bucketed full sweep, not a reduced selector.
