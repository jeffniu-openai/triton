...
Full collection with `PYTHONPATH` unset reports `4998` tests after the 2026-04-14 staged CP, `ld.red`, `ld/st`, MMAv5, and scaled-MMAv5 coverage expansions. The coverage-preserving bucket split is:
...
| `splitn` / misc | exact function nodeids | 273 | split 4, one process per GPU |
...
The buckets sum to all `4998` collected tests. The `splitn` bucket must use exact nodeids; plain `-k splitn` also matches parameter IDs such as `32x32b_splitn` inside `ld_red` and `ld/st`, which pollutes the timing profile.
...
- true exact-nodeid `splitn` / misc bucket: after the 2026-04-14 M64 split-N f32+i32 parity expansion, the bucket contains `273` cases. The latest focused M64 split-N selector passed `42` cases across four groups (`11`, `11`, `11`, and `9`) in `4.79s`, `4.77s`, `5.00s`, and `5.78s`; the prior full splitn/misc runner smoke passed `252` tests across four groups (`63` per group) in `8.95s`, `16.06s`, `15.93s`, and `14.22s` before this expansion.
...
Aggregating the current per-bucket evidence gives full matrix coverage: `4552 passed, 446 skipped` across all `4998` collected cases. This is bucketed evidence from focused/bucket reruns, not a reduced matrix claim; refresh the full runner after shared lowering or major scheduling changes.
