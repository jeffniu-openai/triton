...
Full collection with `PYTHONPATH` unset reports `3294` tests after the 2026-04-14 CP, `ld.red`, and scaled-MMAv5 coverage expansions. The coverage-preserving bucket split is:

| Bucket | Selector | Cases | Scheduling |
| --- | --- | ---: | --- |
| `cp` | `-k cp` | 318 | split 4, one process per GPU |
| `mma` | `-k test_tmem_runtime_matrix_mma` | 311 | split 4, one process per GPU |
...
The buckets sum to all `3294` collected tests. The `splitn` bucket must use exact nodeids; plain `-k splitn` also matches parameter IDs such as `32x32b_splitn` inside `ld_red` and `ld/st`, which pollutes the timing profile.
...
- `cp`: after the 2026-04-14 CP 128x128b dtype expansion, `313 passed, 5 skipped` across four groups in about `50s`, `58s`, `66s`, and `69s` pytest time (`52.6s`, `60.0s`, `68.5s`, `71.9s` runner wall).
...
Aggregating the current per-bucket evidence gives full matrix coverage: `2848 passed, 446 skipped` across all `3294` collected cases. This is a bucketed full sweep, not a reduced selector.
